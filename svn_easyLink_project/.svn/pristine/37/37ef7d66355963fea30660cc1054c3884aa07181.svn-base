...
int writeEMVCAPK_MoreVisa(int *piBlockIndex, int *piBlockPos)
{
	...
	for(i=0; i < 9; i++)
	{
		iRet = PdkFile_Write(iFd, (uchar *)glAllLiveEmvKey[i].KeyValue, sizeof(EMV_CAPK));
	}
...
int WriteTestCAPK(int *piBlockIndex, int *piBlockPos)
{
	...
	************************************************/
	...
	for(i=0; i < 56; i++)
	{
		iRet = PdkFile_Write(iFd, (char *)AllTestEmvKey[i].KeyValue, sizeof(EMV_CAPK));
	}
...
int EMVSetPARAM(void)
{
	...
	EMV_PARAM EmvParam = {0};
	EMV_TERMINALINFO_NODE_T stEmvTermInfoNode = {0};
	EMV_ICS_NODE_T stEmvIcsNode = {0};

	EMV_MCKPARAM 	stEmvMckParam={0};	// Added by ZhangYuan 2016-11-29
	EMV_EXTMPARAM	stEmvExtParam={0};
	...
	memcpy(EmvParam.MerchName, stEmvTermInfoNode.aucMerchName, sizeof(EmvParam.MerchName));
	memcpy(EmvParam.MerchId, stEmvTermInfoNode.aucMerchId, sizeof(EmvParam.MerchId));
	memcpy(EmvParam.TermId, stEmvTermInfoNode.aucTermId, sizeof(EmvParam.TermId));

	memcpy(EmvParam.CountryCode, stEmvTermInfoNode.aucCountryCode, sizeof(EmvParam.CountryCode));
	memcpy(EmvParam.ReferCurrCode, stEmvTermInfoNode.aucReferCurrCode, sizeof(EmvParam.ReferCurrCode));
	memcpy(EmvParam.TransCurrCode, stEmvTermInfoNode.aucTransCurrCode, sizeof(EmvParam.TransCurrCode));

	memcpy(EmvParam.MerchCateCode, stEmvTermInfoNode.aucMerchCateCode, sizeof(EmvParam.MerchCateCode));
	EmvParam.TransCurrExp = stEmvTermInfoNode.ucTerminalCurrencyExponent;

	// if ICS Type parameter not set, use the first ICS.
	EL_LOG(LOG_DEBUG, LOG_TAG, __FUNCTION__, __LINE__, "g_tTransParam.ucICSType=%s\n", g_tTransParam.ucICSType);
	if ( strlen(g_tTransParam.ucICSType) == 0 ) {
		memset(&stEmvIcsNode, 0, sizeof(stEmvIcsNode));
		iRet = GetEmvIcsData(&stEmvIcsNode, sizeof(stEmvIcsNode), 0);
		if ( iRet < 0 ) {
			return iRet;
		}
		EmvParam.Capability[0] = stEmvIcsNode.CardDataInputCapability;
		EmvParam.Capability[1] = stEmvIcsNode.CVMCapability;
		EmvParam.Capability[2] = stEmvIcsNode.SecurityCapability;
		memcpy(EmvParam.ExCapability, stEmvIcsNode.AdditionalTerminalCapabilities, sizeof(EmvParam.ExCapability));
		EmvParam.ForceOnline = stEmvIcsNode.ForcedOnlineCapability;
		EmvParam.GetDataPIN = stEmvIcsNode.GetDataForPINTryCounter;
		EmvParam.TerminalType = stEmvIcsNode.TerminalType;

		// Added by ZhangYuan 2016-11-29, add MCK parameter
		stEmvExtParam.ucBypassAllFlg = (stEmvIcsNode.SubsequentBypassPINEntry == 0) ? 0 : 1;
		stEmvExtParam.ucUseTermAIPFlg = 1;	// Terminal Risk Management is always going to be performed irrespective of the card AIP
		memcpy(stEmvExtParam.aucTermAIP,  "\x08\x00", 2);
		
		stEmvMckParam.ucBypassPin = (stEmvIcsNode.BypassPINEntry == 0) ? 0: 1;
		stEmvMckParam.pvoid = &stEmvExtParam;    		
	} else {
		//ICS
		...
		for(i = 0; i < iNum; i++)
		{
			...
			if ( strcmp(g_tTransParam.ucICSType, stEmvIcsNode.Type) == 0 ) {
				EmvParam.Capability[0] = stEmvIcsNode.CardDataInputCapability;
				EmvParam.Capability[1] = stEmvIcsNode.CVMCapability;
				EmvParam.Capability[2] = stEmvIcsNode.SecurityCapability;
				memcpy(EmvParam.ExCapability, stEmvIcsNode.AdditionalTerminalCapabilities, sizeof(EmvParam.ExCapability));
		...
		//if no one matched. use the first ICS.
		if ( i == iNum ) {
			memset(&stEmvIcsNode, 0, sizeof(stEmvIcsNode));
			iRet = GetEmvIcsData(&stEmvIcsNode, sizeof(stEmvIcsNode), 0);
			if ( iRet < 0 ) {
...
int EMVLoadAppFromFile(void)
{
	int				iRet=0, i=0, iNum=0;
	EMV_APPLIST		stEmvApp = {0};
...
//		stEmvApp.AcquierId;
		memcpy(stEmvApp.AID, stEmvAidNode.AID, sizeof(stEmvAidNode.AID));
		stEmvApp.AidLen = stEmvAidNode.AidLen;
		memcpy(stEmvApp.AppName, stEmvAidNode.AppName, sizeof(stEmvAidNode.AppName));
		memcpy(stEmvApp.dDOL, stEmvAidNode.dDOL, sizeof(stEmvAidNode.dDOL));
		stEmvApp.FloorLimit = stEmvAidNode.FloorLimit;
		stEmvApp.FloorLimitCheck = 1;
		stEmvApp.MaxTargetPer = stEmvAidNode.MaxTargetPer;
		stEmvApp.Priority = 0;
		stEmvApp.RandTransSel = 1;
		memcpy(stEmvApp.RiskManData, stEmvAidNode.RiskManData, sizeof(stEmvAidNode.RiskManData));
		stEmvApp.SelFlag = stEmvAidNode.SelFlag;
		memcpy(stEmvApp.TACDefault, stEmvAidNode.TACDefault, sizeof(stEmvAidNode.TACDefault));
		memcpy(stEmvApp.TACDenial, stEmvAidNode.TACDenial, sizeof(stEmvAidNode.TACDenial));
		memcpy(stEmvApp.TACOnline, stEmvAidNode.TACOnline, sizeof(stEmvAidNode.TACOnline));
		stEmvApp.TargetPer = stEmvAidNode.TargetPer;
		memcpy(stEmvApp.tDOL, stEmvAidNode.tDOL, sizeof(stEmvAidNode.tDOL));
		stEmvApp.Threshold = stEmvAidNode.Threshold;
		stEmvApp.VelocityCheck = 1;
		memcpy(stEmvApp.Version, stEmvAidNode.Version, sizeof(stEmvAidNode.Version));
...
int EMVLoadSpecificApp(unsigned char *pucAid, int iAidLen, EMV_APPLIST *pstEmvApp)
{
	...
	for(i = 0; i < iNum; i++)
	{
		...
		if(memcmp(stEmvAidNode.AID, pucAid, iAidLen) == 0)
		{
//			stEmvApp.AcquierId;
			memcpy(stEmvApp.AID, stEmvAidNode.AID, sizeof(stEmvAidNode.AID));
			stEmvApp.AidLen = stEmvAidNode.AidLen;
...
int EMVLoadSpecificCapk(unsigned char *pucRid, unsigned char ucKeyIndex, EMV_CAPK *pstEmvCapk)
{
	...
	//modified by Kevin Liu 20161115
	...
	for(i = 0; i < iNum; i++)
	{
		...
		iRet = GetEmvCapkData(&stEmvCapkNode, sizeof(stEmvCapkNode), i);
		if ( iRet < 0 ) {
			return iRet;
		}

		if(memcmp(stEmvCapkNode.RID, pucRid, 5) != 0)
		{
			continue;
		}
		if(ucKeyIndex != stEmvCapkNode.KeyID)
		{
			continue;
		}

		memcpy(pstEmvCapk, &stEmvCapkNode, sizeof(EMV_CAPK));
		break;
	}
...
/*
	...
	for(i=0; i < g_stEmvParamInfo.usCapkNum; i++)
	{
		...
		iRet = PdkFile_Read(iFd, (char *)&stEmvCapk, sizeof(EMV_CAPK));
...
int EMVLoadSpecificRevoke(unsigned char *pucRid, unsigned char ucKeyIndex, EMV_REVOCLIST *ptEmvRevokNode)
{
	...
	for(i = 0; i < iNum; i++)
	{
		...
		if(memcmp(stEmvRevokNode.ucRid, pucRid, 5) != 0)
		{
			continue;
		}
		if(ucKeyIndex != stEmvRevokNode.ucIndex)
		{
			continue;
		}

		memcpy(ptEmvRevokNode, &stEmvRevokNode, sizeof(EMV_REVOCLIST));
		break;
	}
...
static int EMVGetCardScheme(unsigned char *pucAid, unsigned char *pucICSTypeOut)
{
	int iRet=0, iNum=0, i=0, iFlag=0;
	EMV_CARD_SCHEM_CONF_T	stEmvCardSchemConfNode;
	uchar aucIcsType[128];
	
	iNum = EmvGetNodeNum(EMV_CARD_SCHEM_CONF_NODE);
	EL_LOG(LOG_DEBUG, LOG_TAG, __FUNCTION__, __LINE__, "EmvGetNodeNum=%d\n", iNum);
	if ( iNum < 0 ) 
	{
		return -1;
	}

	memset(aucIcsType, 0, sizeof(aucIcsType));
	...
	for(i = 0; i < iNum; i++)
	{
		...
		EL_LOG(LOG_DEBUG, LOG_TAG, __FUNCTION__, __LINE__, "GetEmvCardSchemConfData=%d, IcsType: %s\n", iRet, stEmvCardSchemConfNode.ucIcsType);
		if(iRet < 0)
		{			
			return -1;
		}

		if(memcmp(stEmvCardSchemConfNode.ucAid, pucAid, 17) == 0)	// AID
		{
			strcpy(pucICSTypeOut, stEmvCardSchemConfNode.ucIcsType);
			EL_LOG(LOG_DEBUG, LOG_TAG, __FUNCTION__, __LINE__, "AID --- Selected IcsType: %s\n", pucICSTypeOut);
			return 0;
		}

		if(memcmp(stEmvCardSchemConfNode.ucRid, pucAid, 5) == 0)	// RID
		{
			if(iFlag != 1)
			{
				strcpy(aucIcsType, stEmvCardSchemConfNode.ucIcsType);	// first matched RID
			}
			iFlag = 1;
		}
	}

	if(iFlag == 1 && strlen(aucIcsType) > 0)
	{
		strcpy(pucICSTypeOut, aucIcsType);
		EL_LOG(LOG_DEBUG, LOG_TAG, __FUNCTION__, __LINE__, "RID --- Selected IcsType: %s\n", pucICSTypeOut);
		return 0;
	}
...
void EMVLoadCardSchemeConfig(unsigned char *pucAid)
{
	int 			iRet=0, iNum=0, i=0;	
	unsigned char	aucIcsType[128];
	EMV_ICS_NODE_T 	stEmvIcsNode;
	EMV_MCKPARAM 	stEmvMckParam;	// Added by ZhangYuan 2016-11-29
	EMV_EXTMPARAM	stEmvExtParam;

	memset(aucIcsType, 0, sizeof(aucIcsType));
	iRet = EMVGetCardScheme(pucAid, aucIcsType);
	if(iRet != 0)	// not match
	{
		return;
	}

	memset(&stEmvIcsNode, 0, sizeof(EMV_ICS_NODE_T));
	iNum = EmvGetNodeNum(EMV_ICS_NODE);
	EL_LOG(LOG_DEBUG, LOG_TAG, __FUNCTION__, __LINE__, "EmvGetNodeNum=%d\n", iNum);
	if ( iNum < 0 ) 
	{
	...
	for(i=0; i<iNum; i++)
	{
		memset(&stEmvIcsNode, 0, sizeof(stEmvIcsNode));
		iRet = GetEmvIcsData(&stEmvIcsNode, sizeof(stEmvIcsNode), i);
		EL_LOG(LOG_DEBUG, LOG_TAG, __FUNCTION__, __LINE__, "GetEmvIcsData=%d, IcsType: %s\n", iRet, stEmvIcsNode.Type);
		if ( iRet < 0 ) {
			return ;
		}

		if(strcmp(stEmvIcsNode.Type, aucIcsType) == 0)
		{
			EMV_PARAM stEmvParam = {0};

#ifndef _WIN32
			EMVGetParameter(&stEmvParam);
#endif			
			stEmvParam.Capability[0] = stEmvIcsNode.CardDataInputCapability;
			stEmvParam.Capability[1] = stEmvIcsNode.CVMCapability;
			stEmvParam.Capability[2] = stEmvIcsNode.SecurityCapability;
			memcpy(stEmvParam.ExCapability, stEmvIcsNode.AdditionalTerminalCapabilities, sizeof(stEmvParam.ExCapability));
			stEmvParam.ForceOnline = stEmvIcsNode.ForcedOnlineCapability;
			stEmvParam.GetDataPIN = stEmvIcsNode.GetDataForPINTryCounter;
			stEmvParam.TerminalType = stEmvIcsNode.TerminalType;

			EL_LOG_HEX(LOG_DEBUG, LOG_TAG, __FUNCTION__, __LINE__, "Capability: ", stEmvParam.Capability, 3);
			EL_LOG_HEX(LOG_DEBUG, LOG_TAG, __FUNCTION__, __LINE__, "ExCapability: ", stEmvParam.ExCapability, 5);
			EL_LOG(LOG_DEBUG, LOG_TAG, __FUNCTION__, __LINE__, "ForceOnline=%02x, GetDataPIN=%02x, TerminalType=%02x\n", stEmvParam.ForceOnline, stEmvParam.GetDataPIN, stEmvParam.TerminalType);

#ifndef _WIN32
			EMVSetParameter(&stEmvParam);
#endif
			memset(&stEmvMckParam, 0, sizeof(EMV_MCKPARAM));	
			memset(&stEmvExtParam, 0, sizeof(EMV_EXTMPARAM));
			stEmvExtParam.ucBypassAllFlg = (stEmvIcsNode.SubsequentBypassPINEntry == 0) ? 0 : 1;
			stEmvExtParam.ucUseTermAIPFlg = 1;	// Terminal Risk Management is always going to be performed irrespective of the card AIP
			memcpy(stEmvExtParam.aucTermAIP,  "\x08\x00", 2);
...
{
#ifdef DEBUG_SERIAL
    unsigned char timeBuffer[32]={0};
    unsigned char buffer[BUF_LEN_2048]={0};
    va_list ap;

	if(0 == g_UsbOpenFlag)
    {
        PortOpen(COM_USB, "115200,8,n,1");
		g_UsbOpenFlag = 1;
    }
...
void AppHexLogBt(unsigned char *pucMsg, unsigned char *pucData, int iDataLen)
{
#ifdef DEBUG_SERIAL
    unsigned char timeBuffer[8], aucBuf[8+1];
    unsigned char buffer[1024];
    ...
    GetTime(timeBuffer);
    sprintf(buffer, "%02x/%02x/%02x %02x:%02x:%02x -- %s", timeBuffer[0], timeBuffer[1], timeBuffer[2], timeBuffer[3], timeBuffer[4], timeBuffer[5], pucMsg);
	PortSends(7, buffer, strlen(buffer));
	
	if(iDataLen > (1000 - strlen(buffer)))
		iDataLen = 1000 - strlen(buffer);

    for(i=0; i<iDataLen; i++)
	{
		memset(aucBuf, 0, sizeof(aucBuf));
		sprintf(aucBuf, "%02x ", pucData[i]);
		PortSends(7, aucBuf, 3);
	}
