...
static WORD SendBatchHeader_Details(void)
{
    ...
    HDGStruct lsHDGBuf;
    DebugComSend("SendBatchHeader_Details\n");
    ...
    if (gTrHandle >= 0)
    {
		if ( Seek(gTrHandle, 0, SEEK_SET) >= 0 )
        {
            do
            {
				...
				DebugComSend("NumBytes = %d\n", NumBytes);
                if (NumBytes>0)
                {
					DebugComSend("TransType = %s\n", gPTStruct.TransType);
   ...
   StrLeftPad(gCounterPayment,3,lWrkBuf,'0');
    ...
    DebugComSend("lCounterPayment  = %d\n", lCounterPayment);   
    if(lCounterPayment>0)
    {
        ...
        if(sw==STAT_NOK)
        {
			DebugComSend("ISO_RECONCILIATION  = %d\n", sw);
            return(STAT_NOK);
        }
    ...
    {
		DebugComSend("lCounterPayment<=0\n");
        sw = STAT_NOK; //Response Code OK -- FULL
    }
    DebugComSend("lCounterPayment1  = %d\n", lCounterPayment);  
	DebugComSend("sw1  = %d\n", sw);  
...
if(gDebug)
{
                ...
                {
                    Close(gTrHandle);
					DebugComSend("ReceiveStr !=STAT_OK\n");
                    return(STAT_INV);
                }
                ...
                if(lpWrkBuf[5]==5)
                {
                    ...
                    {
                        Close(gTrHandle);
						DebugComSend("ReceiveStr2 !=STAT_OK\n");
                        return(STAT_INV);
                    }
                }
                ...
                {
                    Close(gTrHandle);
					DebugComSend("ReceiveStr3 !=STAT_OK\n");
                    return(STAT_INV);
                }
                ...
                {
                    Close(gTrHandle);
					DebugComSend("ReceiveStr4 !=STAT_OK\n");
                    return(STAT_INV);
                }
                ...
                {
                    Close(gTrHandle);
					DebugComSend("ReceiveStr5 !=STAT_OK\n");
                    return(STAT_INV);
                }
                ...
                do
                {
                    ...
                    if ( NumBytes>0)
                    {
                        ...
                        if (sw==STAT_OK)//Response Code OK -- PARTIAL
                        {
                            if(memcmp(gPTStruct.TransType,TYPE_70,2)==0)//INSTALLMENT
                            {
                               // PrepareField55(OS_TRUE,OS_TRUE);   // removed by Allen 2016-09-05 
                                if(ISO_SendReceive(OS_FALSE,ISO_INSTALLMENT_ADVICES)!=STAT_OK)
                                {
                                    Close(gTrHandle);
									DebugComSend("ISO_SendReceive ISO_INSTALLMENT_ADVICES !=STAT_OK\n");
                                    return(STAT_NOK);
                                }
                            }
                            if(memcmp(gPTStruct.TransType,TYPE_71,2)==0)//LOYALTY
                            {
                                ...
                                {
                                    Close(gTrHandle);
									DebugComSend("ISO_SendReceive ISO_LOYALTY_ADVICE !=STAT_OK\n");
                                    return(STAT_NOK);
                                }
                            }
                        }
                        else//Response Code NOK -- FULL
                        {
                            if(memcmp(gPTStruct.TransType,TYPE_72,2)==0)//PAYMENT
                            {
                                ...
                                {
                                    Close(gTrHandle);
									DebugComSend("ISO_SendReceive ISO_BATCH_UPLOAD_ADVICE != STAT_OK\n");//  can't recive any response from host : Allen Flag
                                    return(STAT_NOK);
                                }
                            }
                            if(memcmp(gPTStruct.TransType,TYPE_70,2)==0)//INSTALLMENT
                            {
                               // PrepareField55(OS_TRUE,OS_TRUE);
                                ...
                                {
                                    Close(gTrHandle);
									DebugComSend("ISO_SendReceive ISO_INSTALLMENT_ADVICES222 !=STAT_OK\n");
                                    return(STAT_NOK);
                                }
                            }
                            if(memcmp(gPTStruct.TransType,TYPE_71,2)==0)//LOYALTY
                            {
                                ...
                                {
                                    Close(gTrHandle);
									DebugComSend("ISO_SendReceive ISO_LOYALTY_ADVICE222 !=STAT_OK\n");
                                    return(STAT_NOK);
                                }
                            }
                        }
        ...
        {
			DebugComSend("ISO_SendReceive ISO_RECONCILIATION_ADVICES222 !=STAT_OK\n");
            return(STAT_NOK);
        }
...
//removed by Allen 20151125
    ...
    if(!Success)       //modify by Allen  20151125
    {
