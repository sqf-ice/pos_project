...
extern WORD EnterPIN(BYTE *PIN);   //added by Allen 2016-08-26

//added by Allen 2016-08-25
...
int ProcMagMsg(void)
{
	...
	if(strlen(Magmsg.track2) > 0)
	{
		...
		//memcpy(Magmsg.track2, "6396790104886002", 16);
		strcpy(gPTStruct.TrackISO2, Magmsg.track2);
	}
	//memcpy(gPTStruct.ExpDate, "4912", 4);
	return iRet;
}
//add end
...
//     }	
    ...
    {
        //PrepareField55(OS_FALSE,OS_FALSE);     //removed by Allen 2016-08-25
...
WORD AlBarakaMerchantConfirm(BYTE step)
{
    ...
    while (1)
    {
        ...
        if (step==1)
        {
            ...
            ScrPrint(0, 0, ASCII, "Prosper like to buy");
        ...
        else if (step==2)
        {
            ...
            ScrPrint(0, 2, ASCII, "following to Prosper");
        } 
        else
        {
            ...
            PubTrimHeadChars(szBuffer, ' ');
            PubTrimTailChars(szBuffer, ' ');
            ScrPrint(0, 0, ASCII, "Prosper like to sell");
...
static WORD StartApplication(BYTE *FirstChar)
{
	...
	//memcpy(gPTStruct.Pan, "6396790104886002", 16);
	//gPTStruct.Pan[16] = 0;
...
*/
   ...
   // memset(gPTStruct.ExpDate,'0',4);   //removed by Allen 2016-08-25
	...
	gEntryType=SWIPE_CARD;      //modified by Allen 2016-08-25
