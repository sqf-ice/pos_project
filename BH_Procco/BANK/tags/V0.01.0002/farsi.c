...
uchar   k_ArShowLine, k_ArDoubleFlg;
ushort  k_ArLineWordWidth, k_ArPrnOffset=0;
...
//        }
        ...
        {
            i     = s_ArFindRhyme(ShiftStr, rIndex+1);
            cForm = s_ArGetForm(ShiftStr[rIndex], BType, TYPE_FARSI_NONE);
            FmtStr[wIndex*2]     = ShiftStr[rIndex];
            FmtStr[wIndex*2 + 1] = cForm;
            wIndex++;
            rIndex++;
        ...
        if(CType==TYPE_EXP || CType==TYPE_NUMERIC)
        {
            ...
            {
                FmtStr[wIndex*2]     = ShiftStr[j-1];
                FmtStr[wIndex*2 + 1] = s_ArGetForm(ShiftStr[j-1], TYPE_NUMERIC, TYPE_FARSI_NONE);
                wIndex++;
            }
            rIndex = i;
            BType  = TYPE_NUMERIC;
...
uint s_ArFindLine(tFontIndex *Head, uchar *FmtStr, uint Index, uint BlankWidth)
{
    ...
    for(i=0; i<1024; i++)
    {
        if(Head[FmtStr[2*CurIndex]].type == TYPE_FARSI_NONE)
        {
            return(EndIndex);
        }
        WordWidth = 0;
        for(j=0; j<1024; j++)
        {
            ...
            if(Head[code].type == TYPE_FARSI_NONE)  break;
            if(!(status & 0x80))
...
////////////////////
    ...
    if(k_FDisp[FmtStr[2*BegIndex]].type == TYPE_FARSI_NONE)
        return(AR_SUCCESS);
    ...
    while(1)
    {
        ...
        if(k_FDisp[code].type == TYPE_FARSI_NONE)   break;
...
uchar s_ArShowStr(uchar Mode, uchar *FmtStr)
{
    ...
    while(1)
    {
        if(k_FDisp[FmtStr[2*BegIndex]].type == TYPE_FARSI_NONE)    return(AR_SUCCESS);
...
{
    uchar   buf[4096], dot[1024], buff[8192];
    uint    fillx, filly, offset;
    uint    fx, iTotalLen, iTmpLen;
    uchar   height, width;
    uint    index, i, j, k, iRet, iStart, iEnd;
    uchar   ucRet, code, status;
    uchar   filloff, tempdot;
////////////////////
//uchar   data[1024];
...
////////////////////
    ...
    if(k_FPrn[FmtStr[2*BegIndex]].type == TYPE_FARSI_NONE)
        return(AR_SUCCESS);

	iTotalLen = PRN_LINE_LEN - k_ArPrnOffset;

    //ucRet = Mode & (~EFT_REVERSE);
    if(Mode &  SHIFT_CENTER)
    {
        fillx = iTotalLen - (iTotalLen-k_ArLineWordWidth)/2;
		iStart = (iTotalLen-k_ArLineWordWidth)/2;
		iEnd = iStart+k_ArLineWordWidth;
    }
    else if(Mode &  SHIFT_LEFT)
    {
        fillx = k_ArLineWordWidth;
		iStart = 0;
		iEnd = k_ArLineWordWidth;
    }
    else 
    {
        fillx = iTotalLen;
		iStart = iTotalLen - k_ArLineWordWidth ;
		iEnd = iTotalLen;
    }
	iStart /= 8; 
	if (iStart>0)
    ...
    while(1)
    {
        ...
        code   = FmtStr[index*2];
        status = FmtStr[index*2+1];
        if(k_FPrn[code].type == TYPE_FARSI_NONE)   break;
...
/////////////////
        ...
        for(i=0; i<height; i++)
        {
            for(j=0; j < (uint)((width+7)/8); j++)
            {
                tempdot = dot[i*((width+7)/8) + j];
                buf[1 + i*(iTotalLen/8+2) + 2 + fx + j]     |= (tempdot>>filloff);
                buf[1 + i*(iTotalLen/8+2) + 2 + fx + j + 1] |= (tempdot<<(8-filloff));
            }
        }
        index++;
        fillx -= width;
    ...
    for(i=0; i<filly; i++)
    {
        buf[1 + i*(iTotalLen/8+2)]     = 0x00;
        buf[1 + i*(iTotalLen/8+2) + 1] = iTotalLen/8;
        if(Mode & EFT_REVERSE)
        {
            //for(j=0; j<PRN_LINE_LEN/8; j++)
            for(j=iStart; j<iEnd; j++)
            {
                buf[1 + i*(iTotalLen/8+2) + 2 + j] = ~buf[1 + i*(iTotalLen/8+2) + 2 + j];
            }
        }
    }

	if (((k_ArDoubleFlg & EFT_DOUBLE_HIGH) == 0) && 
		((k_ArDoubleFlg & EFT_DOUBLE_WIDTH) == 0))
	{
		ucRet = PrnLogo(buf);
	}
	else
	{
		if (k_ArDoubleFlg & EFT_DOUBLE_WIDTH)
		{
			buff[0] = buf[0];
			iTmpLen = iTotalLen/8+2; 
			for(i=0; i<buf[0]; i++)
			{
				buff[1 + i*iTmpLen]     = buf[1 + i*iTmpLen];
				buff[1 + i*iTmpLen + 1] = buf[1 + i*iTmpLen + 1];
				for(j=0; j<iTotalLen/8; j++)
				{
					dot[j*2] = 0;
					tempdot = buf[1 + i*iTmpLen + 2 + j]>>4;
					for (k=0; k<4; k++)
					{
						if ((tempdot>>(3-k)) & 0x01)
						{
					...
					dot[j*2+1] = 0;
					tempdot = buf[1 + i*iTmpLen + 2 + j] & 0x0F;
					for (k=0; k<4; k++)
					{
						if ((tempdot>>(3-k)) & 0x01)
						{
							dot[j*2+1] |= (0x01 << (7-k*2));
							dot[j*2+1] |= (0x01 << (6-k*2));
						}
					}
				}
				if(Mode &  SHIFT_CENTER)
				{
					memcpy(&buff[1+i*iTmpLen+2], &dot[24], iTotalLen/8);
				}
				else if (Mode &  SHIFT_LEFT)
				{
					memcpy(&buff[1+i*iTmpLen+2], dot, iTotalLen/8);
				}
				else
				{
					memcpy(&buff[1+i*iTmpLen+2], &dot[48], iTotalLen/8);
				}
			}
			memcpy(buf, buff, sizeof(buf));
		}
		if (k_ArDoubleFlg & EFT_DOUBLE_HIGH)
		{
			buff[0] = buf[0]*2;
			iTmpLen = iTotalLen/8+2; 
			for(i=0; i<buf[0]; i++)
			{
				memcpy(&buff[1+i*2*iTmpLen], &buf[1+i*iTmpLen], iTmpLen);
				memcpy(&buff[1+(i*2+1)*iTmpLen], &buf[1+i*iTmpLen], iTmpLen);
			}
		}
		ucRet = PrnLogo(buff);
	}
...
uchar s_ArPrnStr(uchar Mode, uchar *FmtStr)
{
    ...
    while(1)
    {
        if(k_FPrn[FmtStr[2*BegIndex]].type == TYPE_FARSI_NONE)    return(AR_SUCCESS);
...
void ArPrintIndent(ushort usValue)
{
	k_ArPrnOffset = usValue;
}

void ArPrnDoubleHigh(uchar ucFlg)
{
	if (ucFlg)
	{
		k_ArDoubleFlg |=  EFT_DOUBLE_HIGH;
	}
	else
	{
		k_ArDoubleFlg &= (~EFT_DOUBLE_HIGH);
...
void ArPrnDoubleWidth(uchar ucFlg)
{
	if (ucFlg)
	{
		k_ArDoubleFlg |=  EFT_DOUBLE_WIDTH;
	}
	else
	{
		k_ArDoubleFlg &= (~EFT_DOUBLE_WIDTH);
