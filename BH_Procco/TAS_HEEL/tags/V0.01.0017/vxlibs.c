...
#endif
//Modified by Joy_ZhouCJ 2014-06-26
unsigned char sAlBarakaLogo_Display[]={
,
...
int main(void)
{
    ...
    while (1) 
    {
        ...
        else if ( (key=IccDetect(0))!=0xFF )
        {
			...
			ScrPrint(0, 5, ASCII, "    TAS'HEEL Card"); 
			ScrPrint(0, 6, ASCII, "      Services");  //Modified by Joy_ZhouCJ 2014-06-25
...
// 	}
	...
	//modified by Joy_ZhouCJ 2014-07-02
   ...
   if (memcmp(gPTStruct.TransType,TYPE_72,2)==0)
    ...
    case ICC_CMD_ERR:
		return 2;
    default:
        if(1 == ucDoNotHonorFlag)   //add by Allen 20151118
        {
            ucDoNotHonorFlag = 0;
            return 2;
        }
...
BYTE Enter_data(BYTE *chTemp,BYTE Type)
{
	...
	switch (Type)
    {
		...
		case 2:	//user
			...
			MainMSG.CSTMSG8[0] = NOCLEARDISP; 
			Min=0;
			Max=LG_USER;	//modified by Kevin v0.01.0017 20150617
			Alpha=1;
			Mask=0;
			break;
		case 3:	//pwd
			...
			MainMSG.CSTMSG8[0] = NOCLEARDISP; 
			Min=0;
			Max=LG_PASSWORD;		//modified by Kevin v0.01.0017 20150617
			Alpha=1;
			Mask=0;
			break;
		case 4:	//port
		...
		if (RetChar = GetOneKey(TO_DEFAULT))
        {
			...
			else if (RetChar==KEYENTER)
			{
				if (cnt)
				{
                    ...
                    if (ucLen<LG_AMOUNT)
                    {
                        memmove(szEnterBuff+LG_AMOUNT-ucLen, szEnterBuff, ucLen+1); //modified by Joy_ZhouCJ 2014-07-04
                        memset(szEnterBuff, '0', LG_AMOUNT-1-ucLen);  
                    }
                    Utils_ConvertAscToBcd(szEnterBuff, RetBuf, 12, 0, PAD_LEFT, '0'); //modified by Joy_ZhouCJ 2014-07-04
					OS_ClearScreen();
					return cnt;
				}
				if (!ZeroInputOK) 
...
void FormatAmt(char *pDest, char *pBuf, char *CurDes, BYTE InputType)
{
    ...
    if (InputType==ASCII_IN)
    {
        ...
        i = strlen(AsciiInBuf);
        if (i<LG_AMOUNT)//modified by Joy_ZhouCJ 2014-07-03
        {
            memmove(AsciiInBuf+12-i, AsciiInBuf, i+1); //modified by Joy_ZhouCJ 2014-07-03
            memset(AsciiInBuf, '0', 12-i);   
        }
		pBuf = AsciiInBuf;
    }
    memset(AsciiOutBuf, 0, sizeof(AsciiOutBuf));
    sprintf(AsciiOutBuf, "000,000,000.00 ");   //modified by Joy_ZhouCJ 2014-07-03
    memcpy(AsciiOutBuf,    pBuf+1,  3);
    memcpy(AsciiOutBuf+4,  pBuf+4,  3);
    memcpy(AsciiOutBuf+8,  pBuf+7,  3);
    memcpy(AsciiOutBuf+12, pBuf+10, 2); //modified by Joy_ZhouCJ 2014-08-28
...
WORD SelectLanguage(void)
{
	...
	ScrPrint(0, 4, CFONT, "1- Arabic");//modified by Joy_Zhoucj 
