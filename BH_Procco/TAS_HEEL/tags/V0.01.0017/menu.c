...
#define NUM_OPTIONS     16 // Modified by Kim_LinHB 2013-3-20 build 0.05.0010
	...
	memcpy(profit+5, gPTStruct.SchemeProfit, 2);    //modified by Joy_ZhouCJ 2014-09-04
	...
	//Div.100
	memset(profit, 0, sizeof(profit));	
	memcpy(profit+4, "\x01\x00", 2);			//modified by Joy_ZhouCJ 2014-09-04
	DivAsciiBCD(Bcd2, Bcd1, profit, 6, BCD_IN, remain);
	//DivBcdNum (Bcd1 , 100 , Bcd2      , 7);
...
WORD Calculate_principal(char *BalanceAmount)
{
#define NUM_OPTIONS     16
	...
	BYTE Bcd3[15];
	BYTE remain[7];
	BYTE PrincipalAmount[NUM_OPTIONS][LG_AMOUNT+1];
	BYTE PrincipalCount[NUM_OPTIONS+1];
	BYTE MENU[20][22];
	BYTE FirstItem,m;
	BYTE lbMenuLine;
	BYTE *lpBuf[20];

	memset(PrincipalAmount, 0, sizeof(PrincipalAmount));
	for (i=0; i<NUM_OPTIONS; i++)
	{
		memset(PrincipalAmount[i],'0',LG_AMOUNT);
	}


	memset(Buffer, 0, sizeof(Buffer));
	memcpy(Buffer, BalanceAmount, LG_AMOUNT);
	StrTrim(Buffer,Buffer,' ');
	StrLeftPad(Buffer, LG_AMOUNT, Buffer, '0');
	Buffer[LG_AMOUNT]=0;
	memset(Amount, 0, sizeof(Amount));
	...
	for (i=0; i<NUM_OPTIONS; i++)
	{
		...
		if(RateValue>0)
		{
			...
			//MAL*T
			memset(Tenure,0,6);
			Tenure[5]=bytebin2bcd(RateValue);
			MulAsciiBCD(Bcd1, Tenure,Amount, 6, BCD_IN);

			memset(profit,0,6);
			profit[5]=bytebin2bcd(10);
			MulAsciiBCD(Bcd2, Bcd1,profit, 6, BCD_IN);
			
			//R*T/12+1
			memset(profit, 0, sizeof(profit));	
			memcpy(profit+5, gPTStruct.SchemeProfit, 2);
			MulAsciiBCD(Bcd3, Tenure,profit, 6, BCD_IN);

			memset(profit,0,6);
			profit[5]=bytebin2bcd(12);
			DivAsciiBCD(Bcd1, Bcd3, profit, 6, BCD_IN, remain);

			memset(profit,0,6);
			profit[4]=bytebin2bcd(1);
			AddAsciiBCD(Bcd3, profit, Bcd1, 6, BCD_IN);


			//Princapal = (MAL x T)/(1+ (R x T)/12)
			DivAsciiBCD(Bcd1, Bcd2, Bcd3, 6, BCD_IN, remain);
			memset(profit,0,6);
			profit[5]=bytebin2bcd(10);
			MulAsciiBCD(Bcd2, Bcd1,profit, 6, BCD_IN);

			StrBcdToAsc(LG_AMOUNT, Bcd2, Buffer);
			memcpy(&PrincipalAmount[j],Buffer,LG_AMOUNT);
			PrincipalCount[j] = RateValue;
			j++;			
		}
	}
	memset(&MENU,0,sizeof(MENU));
	lbCounter=0;
	strcpy(MENU[lbCounter], _T("MAX SHELF AMOUNT"));

	for (i=0; i<NUM_OPTIONS; i++)
	{
		if(memcmp(PrincipalAmount[i],UTIL_FILLZEROS,12)!=0)
		{
			++lbCounter;
			memset(Buffer,0,sizeof(Buffer));
			Buffer[0]=(PrincipalCount[i]/10)+'0';
			Buffer[1]=(PrincipalCount[i]%10)+'0';

			memcpy(RateAmount,PrincipalAmount[i],LG_AMOUNT);RateAmount[LG_AMOUNT]=0;
	...
	if(lbCounter>=1)
    {
        ...
        {
			ScrCls();
			if (KeyBoardValid((PBYTE)"  ! CAUTION !   ",(PBYTE)" MAX PRINCIPLE", (PBYTE)" OPTIONS", (PBYTE)"CANCEL",0, (PBYTE)"PRINT", KEYB_VAL_MASK|KEYB_CANCEL_MASK, 20) == KEYENTER)
			{
				memcpy(gPTStruct.TotalAmount,BalanceAmount,LG_AMOUNT);
				Print_Principal_Slip(lbCounter,(const BYTE **)lpBuf);
			}
		}
