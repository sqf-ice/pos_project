...
// Added by Kim_LinHB 2013-3-15 build 0.06.0016
...
{
    DWORD ldwTemp;
 #pragma warning(disable: 4244) // Added by Kim_LinHB 2013-3-11 build 0.06.0016  
    ldwTemp = DecDebit>> 24;
    HexDebit[0] = ldwTemp;
    ldwTemp = DecDebit>> 16;
    HexDebit[1] = ldwTemp;
    ...
    HexDebit[2] = ldwTemp;
    HexDebit[3] = DecDebit;
#pragma warning(default: 4244) // Added by Kim_LinHB 2013-3-11 build 0.06.0016
}
...
void ConvDate(BYTE *fpBuffer)
{
    ...
    fpBuffer[2] = '/';
	// Modified by Kim_LinHB 2013-10-8 v0.06.0030
...
void DisplayString(BYTE *Str, WORD Timeout)
{
    ...
    DelayMs((unsigned short)Timeout);
}
// Modified by Kim_LinHB 2014-3-11
void DisplayLongString(BYTE *Str, WORD Timeout)
{
	BYTE fpMess1[30]={0}, fpMess2[30]={0}, fpMess3[30]={0};
	BYTE fpMessOther[100]={0}, bTooLong = FALSE; // Added by Kim_LinHB 2014-3-11
	BYTE Len;    
	...
	if(bTooLong)// Added by Kim_LinHB 2014-3-11
		OS_DisplayStr(6, STR_CENTER, (BYTE *)fpMessOther); 
	DelayMs((unsigned short)Timeout);
}
...
// Modified by Kim_LinHB 2014-5-29 fix although it is not used
BYTE Utils_ConvertBcdToBinary(BYTE *in, WORD len, BYTE isWord, BYTE *pOut)
{
	...
	{ 
		a = SWAPLONG(Utils_ConvertBcdToULong(in,len,OS_FALSE));
        memcpy(out,(BYTE *)&a, 4);
		out[4] = 0;
    }
	if(pOut)
...
void Utils_ConvertAscToBcd(BYTE *ASC, BYTE *BCD, unsigned int fpLength,unsigned int lenOut, BYTE padType, BYTE padChar)
{
    ...
    {
        if (ASCIIBuffer[2*lbCounter]>0x39)
		{
			sub1=0x37; 
		}
		else 
        ...
        if (ASCIIBuffer[2*lbCounter+1]>0x39)
		{
			sub2=0x37; 
		}
		else 
...
void MulAsciiBCD(BYTE * pDest, BYTE * pNum1, BYTE * pNum2, BYTE Len, BYTE InputType)
{
    ...
    if (IsNulls(pNum1, (unsigned short) Len) || IsNulls(pNum2, (unsigned short) Len))	// Either of the multipliands is zero.
        return;
...
/*
	if(i>=0.5)
	{
		...
		if (Len == 6)
			...
			Bin2Bcd_Word(BCD2Ascii_Out, (UINT) Result);
		else if (Len == 2)
			Bin2Bcd_Byte(BCD2Ascii_Out, (BYTE) Result);
		else
			return;
...
{
	int i;
	unsigned long  iDiv = 0;
	unsigned long iByDiv = 0;
	unsigned long iRemain = 0;
	unsigned long iValue = 0;
	unsigned char * ptr;

	ptr = valueStr;
	iDiv = atol(DivStr);
	iByDiv = atol(byDivStr);

	iRemain = iByDiv % iDiv;
	iValue = iByDiv / iDiv;
	
	sprintf((char *)ptr, "%d", iValue);
	ptr += strlen(ptr);
// 	if (mode == 0)
// 	{
// 		// amount/amount
// 		sprintf((char *)decimalStr, "%s%dd", "%0", iPrecision);
// 		sprintf((char *)ptr, decimalStr, 0);
// 	}
//	ptr += strlen(ptr);
	
	for (i = 0; i < iPrecision; i++)//iRemain && 
	{
		iRemain *= 10;
		iValue = iRemain / iDiv;
		*ptr++ = iValue + '30';
		...
		if (iDiv <= iRemain)
		{
			iRemain = iRemain % iDiv;
		}
	}

	iValue = (atol(valueStr) + 5)/10;
	iRemain = iValue%100;
	sprintf((char *)valueStr, "%012d", iValue);
