...
#define filePiece 2048
	...
	if(fexist((char *)fpDstFile) > 0)
		Delete(fpDstFile);
	fd = Open((char *)fpSrcFile, O_RDWR);
	if(fd < 0)
	{
		return (STAT_NOK);
	}
	fd_tmp = Open((char *)fpDstFile,O_CREATE | O_RDWR);
	if(fd < 0)
	{
		return (STAT_NOK);
	}

	fileSize = filesize((char *)fpSrcFile);
	if(fileSize>filePiece)// 2KB each time
	{
		int n = fileSize / filePiece;// n blocks
