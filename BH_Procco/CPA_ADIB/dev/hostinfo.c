...
WORD HostInfoEntry(void)
{
    ...
    BYTE lpUser[32] = {0};
    BYTE lpPwd[32] = {0};
    BYTE lpPort[4+1] = {0};	// Modified by Kim_LinHB 2014-5-29
    BYTE lpTmsNii[5] = {0};
    BYTE lpPmsNii[5] = {0};
...
//we dont require end of day for that
	...
	ScrPrint(0, 4, ASCII, "MERCH ID : 01101");
	ScrPrint(0, 5, ASCII, "STORE ID : 1");
	ScrPrint(0, 6, ASCII, "TERM ID  : 02020205");//01010103 is for CPA test   02020202 is for ADIB test		02020204 is for ADIB test
	DelayMs(2000);
	strcpy(lpMerchId, "01101");
	strcpy(lpStoreNum, "1");
	strcpy(lpTermId, "02020205");//01010103 02020202
	ScrCls();
...
WORD SecurityFunctions(void)
{
	...
	{
		lbMenuLine=KeyBoardMenu(5,gSecurityFnctMenu,&FirstItem,gOldSubItem,200,0);
