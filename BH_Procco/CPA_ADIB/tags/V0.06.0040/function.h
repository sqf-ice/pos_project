...
WORD      Prepare_Print_SaleOrPaymentSlip(BYTE PrintMerhantCopy);	//printout 4
...
WORD FileCopy						   (BYTE *fpDstFile, BYTE *fpSrcFile);
...
//printing action + data length + data
//PRN_ACT_ENG + length(3 Bytes) + data
...
WORD Print_Confirmation();		//printout 2
WORD Print_SaleContract(BYTE PrintMerhantCopy);		//printout 3
WORD Print_SaleContractSub(BYTE PrintMerhantCopy);		//printout 3
#endif
