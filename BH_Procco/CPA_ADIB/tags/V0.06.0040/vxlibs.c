...
WORD Enter_prod_type(unsigned char *psProdType)
{
	...
	if (ARABIC == gLanguage)
	{
		strcpy(stMsg.CSTMSG1, "Ê’› «·»÷«⁄… ");
		//strcpy(stMsg.CSTMSG1, "Ê’› «·»÷«⁄… ");
		//strcpy(stMsg.CSTMSG2, "—ﬁ„ :");
	}
	else
	{
		//strcpy(stMsg.CSTMSG1, "TYPE OF GOOD(S)");
		strcpy(stMsg.CSTMSG1, "DESCRIPTION OF");
		strcpy(stMsg.CSTMSG2, "PRODUCT");
	}
...
//	sprintf(stMsg.CSTMSG2, "FREE TEXT UP TO %dCH", (int)LG_PRODUCT_TYPE);
	...
	if (ARABIC == gLanguage)
	{
		//if (!InputAlphaNum_PAX(&stMsg, psProdType, 0, LG_PRODUCT_TYPE, OS_FALSE, JUSTIFY_CENTER, 30))
		// Modified by Kim_LinHB 2013-5-2 build 0.06.0024 from false to true
		if (!InputAlphaNum_PAX(&stMsg, psProdType, 1, LG_PRODUCT_TYPE, OS_TRUE, JUSTIFY_CENTER, 30))
		{
			return STAT_NOK;
		}
	}
...
//	int iLastKey,iIndex;
	...
	if (ARABIC == gLanguage)
	{
		strcpy(stMsg.CSTMSG1, "—„“ «·‘—Ìÿ");
	}
	else
	{
//		strcpy(stMsg.CSTMSG1, "BARCODE        ");
//		strcpy(stMsg.CSTMSG2, "SCAN BARCODE");
//		strcpy(stMsg.CSTMSG3, "OR MANUALLY KEY IN");
		strcpy(stMsg.CSTMSG1, "BARCODE NUMBER");
	}
...
//  ScrPrint(0, 3, ASCII, "OR MANUALLY KEY IN");
    ...
    kbflush();
    //if (!InputAlphaNum_PAX(&stMsg, psBarcode, 0, LG_PRODUCT_TYPE, OS_TRUE, JUSTIFY_CENTER, 30))
	// Modified by Kim_LinHB 2013-5-2 build 0.06.0024 from False to true
 	if (!InputAlphaNum_PAX(&stMsg, psBarcode, 1, LG_PRODUCT_TYPE, OS_TRUE, JUSTIFY_CENTER, 30))
     {
         return STAT_NOK;
     }
#if 0
...
//by jarod 2011.06.13
//Shouldn°Øt accept entry of characters other than number (0..9) and letters (A..Z) or (a..z) in the barcode entry.
#if 1
			if( ucKey==KEYALPHA )
			{
...
WORD Enter_Item_Price(BYTE *psPriceBcd)
{
	...
	if (ARABIC == gLanguage)
	{
	    strcpy(MainMSG.CSTMSG1, "”⁄— «·ÊÕœ…");
	}
	else
	{
		strcpy(MainMSG.CSTMSG1, "UNIT PRICE");
	}
//    strcpy(MainMSG.CSTMSG1, "   ITEM PRICE   ");
	
	OS_ClearScreen();
    memset(chTemp, 0, sizeof(chTemp));
#ifdef DEVELOP_VERSION
	memcpy(gCurrencySymbol, "AED", 3);
#endif
    cnt = InputAmountExt(&MainMSG, chTemp, 0, 1, 9, gCurrencySymbol);
	if (!cnt)
    {
		return(STAT_NOK);
    }
...
WORD Enter_Item_num(WORD *pwNum)
{
	...
	if (ARABIC == gLanguage)
	{
		strcpy(MainMSG.CSTMSG1, "«·ﬂ„Ì…");
	}
	else
	{
	    strcpy(MainMSG.CSTMSG1, "    QUANTITY    ");
	}
...
//    ScrPrint(0, 2, CFONT|REVER, " ANOTHER ITEM ? ");
	if (ARABIC == gLanguage)
	{
		ArShowString(0, SHIFT_CENTER, "Â·  Êœ ≈÷«›… ");
		ArShowString(2, SHIFT_CENTER, "»÷«∆⁄ √Œ—Ï");
		ArShowString(4, SHIFT_RIGHT, "1- ‰⁄„ ");
		ArShowString(6, SHIFT_RIGHT, "2- ·«");
	}
	else
	{
		ScrPrint(0, 0, ASCII, "WOULD YOU LIKE TO ADD");
		ScrPrint(0, 1, ASCII, "MORE PRODUCTS?");
		
		ScrPrint(0, 4, ASCII, "   [1] - YES");
		ScrPrint(0, 6, ASCII, "   [2] - NO");
	}
...
//	remove("AGENTPIN");
	if (fexist("AGENTPIN") >= 0)
	{
		return STAT_OK;
	}

	memcpy(pszOut, "41234", 5);
...
int main(void)
	{
//	BYTE szDisplayStr[100];
	BYTE timebuffer[7];//for ADIB test
	BYTE pszOut[12];
	BYTE Month,Day,Hour;
...
//	ST_FONT stFontTmp;
	...
	// Added by Kim_LinHB 2013-10-9 0.06.0031
#ifdef APPL_IP
	kbmute(0);
#endif

	if (InitREP_PIN() != STAT_OK)
	{
		ScrCls();
		ScrPrint((BYTE)(64 - 4*15), 3, CFONT, "PLS SET REP.PIN");
		DelayMs(2000);
	}
    ...
    while (1) 
    {
        if (TimerCheck(0)==0)
        {
			...
			sprintf(AutoPassword,"%02x%02d%02d%02d%02x", timebuffer[0], Month, Day, Month+Day, Hour);
        }
        ...
        {//
            ...
            Create_Update_MVTFile(index_arr, &index_len);
                        //remove(EmvESTFile);
			...
			if (gLanguage == ARABIC)
			{
				ArShowString(0, SHIFT_CENTER, "√Â·« »ﬂ„ ›Ì");

//				ArShowString(0, SHIFT_RIGHT, "” ‘ ” ‘ ” ‘ ” ‘ ” ‘ ” ");//17„‰  ﬂ·› Â« ·Â ‘‘‘ 
//				ArShowString(2, SHIFT_RIGHT, "„‰  ﬂ·› Â« ·Â ‘‘");//16
//				ArShowString(4, SHIFT_RIGHT, "„‰  ﬂ·› Â« ·Â ‘‘‘‘");//18

				ArShowString(2, SHIFT_CENTER, "»—‰«„Ã «· „ÊÌ· ");
				ArShowString(4, SHIFT_CENTER, "«·›Ê—Ì „‰ „’—›");
				ArShowString(6, SHIFT_CENTER, "√»ÊŸ»Ì «·≈”·«„Ì");
//				DelayMs(5000);
				
//				ScrCls();
			//	sprintf((char *)szDisplayStr, "%s", "012345678901234567890123456789");
			//	ArShowString(0, SHIFT_RIGHT, szDisplayStr);
//				sprintf((char *)szDisplayStr, "%30d", 12);
//				sprintf((char *)szDisplayStr, "%21.21s", "AED@1234567890 12345 122");
//
//				ArShowString(0, SHIFT_LEFT, szDisplayStr);
//				getkey();
			}
			else
			{
			...
			//	ScrPrint(0, 4, ASCII, "     Welcome To");
			//	ScrPrint(0, 5, ASCII, "Taqseet Card Services");
			//	ScrPrint(0, 6, ASCII, "        from");
			//	ScrPrint(0, 7, ASCII, "AlBaraka Islamic Bank");
			//	ScrPrint(0, 2, ASCII, "1234567890123456789012345");21

				ScrPrint(0 , 2, ASCII, "  WELCOME TO INSTANT");
				ScrPrint(0, 4, ASCII, " FINANCE PROGRAM FROM");
...
//chenxi find here,most of the screens displays from here
...
#endif
	//TransactionType // add by chens for Account Maint 2014.05.12
...
// 	if (inVXEMVAPCardInit() != SUCCESS) return(2);
	...
	//add by chens for Account Maint 2014.05.12
	if(memcmp(gPTStruct.TransType,TYPE_73,2)==0)//only Account Maint need change TACDenial[2]
	{
		EMV_APPLIST app = {0};
		int i = 0;
		for(i=0;i< MAX_APP_NUM;i ++)
		{
			...
			{
				app.TACDenial[2] = (app.TACDenial[2] & 0xDF);//set bit6 to 0 
				EMVAddApp(&app);
			}
		}
	}
...
//         return (2);
	...
	// Modified by Kim_LinHB 2013-10-9 0.06.0031 remove 0, to reject bypass
...
//     }
   ...
   // Added by Kim_LinHB 2014-5-22
   // it's from cEMVOnlineProc() which is incorrect to put it there
   if (EMV_OK == iRet && memcmp(gPTStruct.TransType,TYPE_70,2)==0)
   {
	   uchar	ucKey;
	   uchar	pszOut[12];
	   WORD    usRet;
	   uchar	i;
	   int		fp;
	   Print_SaleContract(OS_TRUE);	//printout 3

	   if (gLanguage != ARABIC)
	   {
			//screen 45
			ScrCls();
			ScrPrint(0, 0, ASCII, "AS ADIB REP. I HEREBY");
			ScrPrint(0, 1, ASCII, "CONFIRM ON MY");
			ScrPrint(0, 2, ASCII, "CAPACITY THAT I HAVE ");
			ScrPrint(0, 3, ASCII, "DELIVERED THE");
			ScrPrint(0, 4, ASCII, "PRODUCTS TO THE");
			ScrPrint(0, 5, ASCII, "I.F.C PHYSICALLY");
			ScrPrint(0, 6, ASCII, "OR CONSTRUCTIVELY:");
			while (1)
			{
				ucKey = GetOneKey(TO_1MIN);
				if (ucKey == KEYCANCEL || ucKey == NOKEY)
				{
					/*after doing successful retail when the POS asks for entering button for copy of slips, if you press cancel then it shows card block
					it confuses the merchant and customer*/
					goto POSITION_APPROVED; // Added by Kim_LinHB 2014-5-21
					//return ONLINE_APPROVE;//STAT_INV;modified by chens 2014.04.24
				}
			...
			//screen 44
			ScrCls();
			ScrPrint(0, 0, ASCII, "1- PHYSICALLY");
			ScrPrint(0, 2, ASCII, "2- CONSTRUCTIVELY");
			while (1)
			{
				ucKey = GetOneKey(TO_1MIN);
				if (ucKey == KEYCLEAR || ucKey == NOKEY)
				{
					goto POSITION_APPROVED; // Added by Kim_LinHB 2014-5-21
					//return ONLINE_APPROVE;//STAT_INV;modified by chens 2014.04.24
				}
			...
			ScrPrint((BYTE)(64 - 3 * 17), 3, ASCII, "PLEASE ENTER YOUR");
			ScrPrint((BYTE)(64 - 3 * 14), 4, ASCII, "ADIB REP. PIN:");
		}
		else
		{
			//screen 44
			ScrCls();
			ArShowString(0, SHIFT_RIGHT, "ﬂ„„À· ··„’—›° ");//pos screen 46
			ArShowString(2, SHIFT_RIGHT, "√ƒﬂœ √‰‰Ì ﬁœ ﬁ„  ");
			ArShowString(4, SHIFT_RIGHT, "» ”·Ì„ «·»÷«⁄… ");
			ArShowString(6, SHIFT_RIGHT, "·„ ⁄«„· «· „ÊÌ· ");
			while (1)
			{
				ucKey = GetOneKey(TO_1MIN);
				if (ucKey == KEYCANCEL || ucKey == NOKEY)
			...
			ScrCls();
			ArShowString(0, SHIFT_RIGHT, "«·›Ê—Ì  ”·Ì„« ");
			ArShowString(2, SHIFT_RIGHT, "ÕﬁÌﬁÌ« √Ê Õﬂ„Ì«");
			...
			ScrCls();
			ArShowString(0, SHIFT_RIGHT, "1-  „ «· ”·Ì„ ");
			ArShowString(2, SHIFT_RIGHT, "  ”·Ì„« ÕﬁÌﬁÌ« ");
			ArShowString(4, SHIFT_RIGHT, "2-  „ «· ”·Ì„ ");
			ArShowString(6, SHIFT_RIGHT, " ”·Ì„« Õﬂ„Ì«");
		...
		{
			ScrGotoxy(52, 6);	
			usRet = GetString(pszOut, 0x6D, 4, 4);//AutoPassword + 4
			if(usRet != 0) goto POSITION_APPROVED; // Added by Kim_LinHB 2014-5-21//return -1;
		...
		Prepare_Print_SaleOrPaymentSlip(OS_TRUE);	//printout 4
		// Added by Kim_LinHB 2013-4-15 build 0.06.0023
		while (1)
		{
			ucKey = GetOneKey(TO_1MIN);
			if (ucKey == KEYCANCEL || ucKey == NOKEY)
...
POSITION_APPROVED: 
   // Modified by Kim_LinHB 2014-5-22
    switch(iRet)
    {
    case EMV_OK:    // approved online or offline
		// Added by Kim_LinHB 2014-4-11
		FileCopy(FILE_R1, FILE_RT);
		FileInit(FILE_RT);
		// Add End
        break;
    case EMV_TIME_OUT:
		OS_ClearScreen();
		DisplayLongString_S80(DSP_TIMEOUT, 5000);
		Print_HostErr_Slip(PRT_TIMEOUT);
		break;
    case EMV_USER_CANCEL:
		OS_ClearScreen();
		DisplayLongString_S80(DSP_USERCANCELLED, 5000);
		Print_HostErr_Slip(PRT_USERCANCELLED);
		break;
    default:
        OS_ClearScreen();
	...
	if(iRet != EMV_OK){
		EMVGetTLVData(0x9F34, gPTStruct.Balance, &iRet);
		EMVGetTLVData(0x95, gPTStruct.Balance, &iRet);
		EMVGetTLVData(0x9F33, gPTStruct.Balance, &iRet);
		EMVGetTLVData(0x9B, gPTStruct.Balance, &iRet);
		EMVGetTLVData(0x9F08, gPTStruct.Balance, &iRet);
		EMVGetTLVData(0x9F09, gPTStruct.Balance, &iRet);
#ifdef DEVELOP_VERSION
...
// 	memcpy(mvt_rec.szDefaultTDOL,"9F02065F2A029A039C0195059F3704",30);
    ...
    */
	...
	return TRUE; // Added by Kim_LinHB 2013-3-15 build 0.06.0016
}
...
int dsp_func(char *value,int mx,int mn,int typ,int punc,int frac)
{
    ...
    return iRetVal;
    */
	return 0; // Added by Kim_LinHB 2013-3-15 build 0.06.0016
}
...
BYTE InputAlphaNum(MSG_Struct *stMSG, char *RetBuf, BYTE MinLen, BYTE MaxLen, BYTE AlphaOk, BYTE MaskOk, BYTE InputAlignment, BYTE TOSeconds)
{
    ...
    {
		memcpy(stMSG->CSTMSG2, "ENTER  ", 7);						// Default String for "Input Type Name" is "G›R›N›Z"
    }
                    ...
                    else if (RetChar == KEY0)	// Signs.
                    {
					    if (cnt < MaxLen)
                        {
                            ...
                            else
                            {
							    AlphaSituation = 0;						// Reset AlphaSituation
							    PrevChar = RetChar;
							    stMSG->CSTMSG7[cnt++] = SignTab[AlphaSituation];// Girilen deeri al.
							    AlphaSituation++;						// Increase Next Situation.
							    AlphaSituation %= NR_OF_SIGNS;			// To maintain cyclic consistency
						    }
					    }
                       ...
                       BYTE AlphaOk, BYTE InputAlignment, BYTE TOSeconds)
{
	...
	BYTE FormatFlag = NOFORMAT;		
	uchar RetChar = 0;  // Modified by Kim_LinHB 2013-3-26 build 0.6.0021 from char to uchar
	char cnt = 0;
	BYTE StrLen = strlen(stMSG->CSTMSG1), TempLen = 0;
	BYTE LineMax/*, Offset,Src*/;
	BYTE AlphaNumState = NUM_STATE;				// Default state.
	...
	if (stMSG->CSTMSG1[0])		// Header Line, if exists
    {
		...
		if (ARABIC == gLanguage)
		{
			ArShowString(0, SHIFT_CENTER, stMSG->CSTMSG1);
		}
		else
		{
...
//by jarod 2011.05.26
	if (stMSG->CSTMSG2[0])
	{
		if (ARABIC == gLanguage)
		{
			ArShowString(2, SHIFT_CENTER, stMSG->CSTMSG2);
		}
		else
		{
			StrLen = (BYTE)strlen(stMSG->CSTMSG2);
			if (StrLen>LARGE_FONT_LINE_MAX)
			{
				ScrPrint(0, 2, ASCII, "%s",stMSG->CSTMSG2);
			}
			else
			{
				ScrPrint(0, 2, CFONT, "%s", stMSG->CSTMSG2);
			}
		}
	}
    ...
    { 
        ...
        ScrPrint(0, 3, ASCII, "%s", stMSG->CSTMSG3);
	}
	...
	//for test ADIB
	if (!stMSG->CSTMSG2[0] || !stMSG->CSTMSG3[0] || !stMSG->CSTMSG4[0])
	{
		line = 4;
	}
	else
	...
	if (ARABIC != gLanguage)
	{
		if (AlphaOk)// If alpha input allowed.
		{
			if (AlphaNumState == ALPHA_STATE)
			{
				display_at(0, line, "         (A)", NO_CLEAR );// Alpha input mode
			}  
			else			
			{
				display_at(0, line, "         (N)", NO_CLEAR );// Numeric input mode
			}
		}
		else if (1)
		{
			...
			if (RetChar != NOKEY)
			{
				...
				{							
					...
					{											// Change AlphaNumState if Alpha allowed
						...
						// Added by Kim_LinHB 2013-5-2 build 0.06.0024 for display
						if (AlphaNumState == ALPHA_STATE)
						{
							display_at(0, line, "         (A)", NO_CLEAR );// Alpha input mode
						}  
						...
						else if (RetChar == KEY0)	// Signs.
						{
							if (cnt < MaxLen)
							{
								...
								else
								{
									AlphaSituation = 0;						// Reset AlphaSituation
									PrevChar = RetChar;
									sEnterBuffer[cnt++] = SignTab[AlphaSituation];// Girilen deeri al.
									AlphaSituation++;						// Increase Next Situation.
									AlphaSituation %= NR_OF_SIGNS;			// To maintain cyclic consistency
								}
							}
				...
				break;	//jump to control display
			}//end if(RetChar != NOKEY)
//         else
//         {
// 		    return 0x00;// TIMEOUT ---
//         }

			ucRet = PortRecv(PINPAD, sEnterBuffer+cnt, 50);
			if (ucRet == 0)
			{
				cnt++;
				while (2)
				...
				if ((ucRet==0) && (strlen(sEnterBuffer)))
				{
					s_display_barcode(sEnterBuffer);
					sprintf(RetBuf, "%.13s", sEnterBuffer);
					DelayMs(500);
					if (!cnt)	// If entered with no input (ZeroInputAllowed case)
					{
						RetBuf[0] = 0x00;// Clear first val, which will be read.
				...
				iRecvCnt = 0;
				memset(RetBuf, 0, MaxLen); // Modified by Kim_LinHB 2014-5-29
...
#define CHOICES_PER_PAGE128_TITLE	4
...
int ShowMenu2(menu *m, BYTE TOSeconds, BYTE OptByte)
{
	MSG_Struct stMSG;
	BYTE ChoicesPerPage;
	BYTE CurrentChoice = 0, CurrentFrameStart = 0, KeyGot/*, LineMax*/;
	...
	ulong SeqKeyTO = 0;													// Sequential key press timeout.
	ulong TouchPadTO = 0;												// Touchpad double click timeout.
	const char IndexTab[LG_MENUINDEX][3] = 
...
//         else
		...
		display_at(0, 4, stMSG.CSTMSG2, NO_CLEAR );
	//	display_at(0, 1, stMSG.CSTMSG2, NO_CLEAR );
	//	display_at(0, 2, stMSG.CSTMSG2, NO_CLEAR );
...
//             else
			...
			display_at(0, 5, stMSG.CSTMSG3, NO_CLEAR );
		//	display_at(0, 2, stMSG.CSTMSG3, NO_CLEAR );	
		//	display_at(0, 3, stMSG.CSTMSG3, NO_CLEAR );	
		}
        else
...
//          else
			...
			display_at(0, 6, stMSG.CSTMSG4, NO_CLEAR );
			//display_at(0, 3, stMSG.CSTMSG4, NO_CLEAR );	
		//	display_at(0, 4, stMSG.CSTMSG4, NO_CLEAR );	
		}
        else
...
// 			else
			...
			display_at(0, 7, stMSG.CSTMSG5, NO_CLEAR );
			//display_at(0, 4, stMSG.CSTMSG5, NO_CLEAR );	
			//display_at(0, 5, stMSG.CSTMSG5, NO_CLEAR );
		}
        else
...
{
	//char    RetChar = (char)0xFF; it's a bug, this bug leads to no returning when timing out
	unsigned char    RetChar = 0xFF;
	BYTE    cnt;
	BYTE    ucLen;
	...
	if (stMSG->CSTMSG2[0])	// Input Type Name 
    {
		...
		else
		{
			if (ucLen>LARGE_FONT_LINE_MAX)
			{
				ScrPrint((128-ucLen*6)/2, 2, ASCII, "%.21s", stMSG->CSTMSG2);
			}
			else
			{
				ScrPrint((128-ucLen*8)/2, 2, CFONT, "%.16s", stMSG->CSTMSG2);
			}
		}
	}
	...
	if (stMSG->CSTMSG3[0])	// If exists
    {
		...
		if (ARABIC == gLanguage)
		{
			ArShowString(4, SHIFT_CENTER, stMSG->CSTMSG3);
		}
		else
		{
			if (ucLen>LARGE_FONT_LINE_MAX)
			{
				ScrPrint((128-ucLen*6)/2, 4, ASCII, "%.21s", stMSG->CSTMSG3);
			}
			else
			{
				ScrPrint((128-ucLen*8)/2, 4, CFONT, "%.16s", stMSG->CSTMSG3);
			}
		}
	}
	...
	while (RetChar != 0)
    {
		...
		if (RetChar = GetOneKey(TO_DEFAULT))
        {
			if ((RetChar >= KEY0) && (RetChar <= KEY9))
            {
				if (cnt < maxlen)
			...
			else if (RetChar==KEYENTER)
			{
				...
				// it's a bug that leads to "00000000000" being converted to "0D0D0D0D0D0D"
			//	Utils_ConvertAscToBcd(szEnterBuff, RetBuf, 12, 6, PAD_LEFT, 0);
				PubAddHeadChars(szEnterBuff, 12, '0');
				PubAsc2Bcd(szEnterBuff, 12, RetBuf);
				OS_ClearScreen();
				return ((cnt == 0) ? 1 : cnt);
			}
		} 
...
//	ST_FONT stFontTmp;
	...
	OS_ClearScreen();	
 	ScrPrint(64 - strlen("Please select") * 4, 0, CFONT, "Please select");
 	ScrPrint(64 - sizeof("language") * 4, 2, CFONT, "language:");

//     ScrPrint(0, 0, CFONT, "SELECT PRINTOUT");
//     ScrPrint(0, 2, CFONT, "LANGUAGE:");
	ArShowString(4, SHIFT_RIGHT, "1-⁄—»Ì "); //Araline                                                                                                                                                                       
	ScrPrint(128 - strlen("English-2") * 8, 6, CFONT, "English-2");
...
//chenxi find here to display tansaction
WORD AskYesNo2(MSG_Struct *stMSG,BYTE Type,BYTE offset) 
{ 
	BYTE /*key,StrLen,*/linemsg[50],SMALL_FONT_MAX;
//	BYTE ucKey;
	int i = 0;
	BYTE szDispBuff[32];
	BYTE szDispBuffTmp[32];
//	BYTE ucLen;
//	BYTE profitRate[2];
//	BYTE ascRate[6];
//	BYTE Bcd2[2];
...
//	BYTE result;
	...
	BYTE DivAmout[13];
	BYTE ByDivAmout[13];
	BYTE PercentAmout[13];
	BYTE DecimalAmout[13];
	DWORD ulRet;
...
//	OS_ClearScreen(); 
	...
	if (ARABIC == gLanguage)
	{
		for (i = 0; i < gPTStruct.TotItems; i++)
		{
			...
			strcpy((char *)stPageContent.stContent[0].stLineContent[0].content, "⁄“Ì“Ì «· «Ã—..");//pos screen 1
			strcpy((char *)stPageContent.stContent[0].stLineContent[1].content, "≈‰ ﬁÌ«„ﬂ„ »≈—”«·");
			strcpy((char *)stPageContent.stContent[0].stLineContent[2].content, "√„— «·‘—«¡ Â–« ");
			strcpy((char *)stPageContent.stContent[0].stLineContent[3].content, "≈·Ï «·„’—› ÂÊ");

			strcpy((char *)stPageContent.stContent[1].stLineContent[0].content, "„Ê«›ﬁ… „‰ﬂ„ ⁄·Ï ");
			strcpy((char *)stPageContent.stContent[1].stLineContent[1].content, "›”Œ √Ì… ⁄·«ﬁ… ");
			strcpy((char *)stPageContent.stContent[1].stLineContent[2].content, " ⁄«ﬁœÌ… ﬁœ  ﬂÊ‰");
			strcpy((char *)stPageContent.stContent[1].stLineContent[3].content, "ﬁ«∆„… »Ì‰ﬂ„ Ê»Ì‰");

			strcpy((char *)stPageContent.stContent[2].stLineContent[0].content, "„ ⁄«„· «· „ÊÌ· ");
			strcpy((char *)stPageContent.stContent[2].stLineContent[1].content, "«·›Ê—Ì »‘√‰");
			strcpy((char *)stPageContent.stContent[2].stLineContent[2].content, "«·»÷«∆⁄ «·„–ﬂÊ—…");
			strcpy((char *)stPageContent.stContent[2].stLineContent[3].content, "Ê≈–« ﬂ«‰ „ ⁄«„·");

			strcpy((char *)stPageContent.stContent[3].stLineContent[0].content, "«· „ÊÌ· «·›Ê—Ì");
			strcpy((char *)stPageContent.stContent[3].stLineContent[1].content, "ﬁœ œ›⁄ Ã“¡« „‰");
			strcpy((char *)stPageContent.stContent[3].stLineContent[2].content, " ﬂ·› Â« ·ﬂ„ √Ê");
			strcpy((char *)stPageContent.stContent[3].stLineContent[3].content, "”Ìœ›⁄ ·ﬂ„ Ã“¡« ");

			strcpy((char *)stPageContent.stContent[4].stLineContent[0].content, "„‰  ﬂ·› Â« ›ﬁœ ");
			strcpy((char *)stPageContent.stContent[4].stLineContent[1].content, "Ê«›ﬁ „ ⁄·Ï ›”Œ ");
			strcpy((char *)stPageContent.stContent[4].stLineContent[2].content, " ·ﬂ «·⁄·«ﬁ… »‘√‰");
			strcpy((char *)stPageContent.stContent[4].stLineContent[3].content, "Õ’… ‘«∆⁄… „‰");

			strcpy((char *)stPageContent.stContent[5].stLineContent[0].content, " «·»÷«∆⁄ ›ﬁÿ ");
			strcpy((char *)stPageContent.stContent[5].stLineContent[1].content, " ”«ÊÌ ‰”»… «·„»·€");
			strcpy((char *)stPageContent.stContent[5].stLineContent[2].content, "€Ì— «·„œ›Ê⁄ ≈·Ï ");
			strcpy((char *)stPageContent.stContent[5].stLineContent[3].content, "≈Ã„«·Ì  ﬂ·›… ");

			strcpy((char *)stPageContent.stContent[6].stLineContent[0].content, "«·»÷«∆⁄. ");//pos screen 7


			strcpy((char *)stPageContent.stContent[7].stLineContent[0].content, " ⁄“Ì“Ì «· «Ã—° ");//pos screen 8
			strcpy((char *)stPageContent.stContent[7].stLineContent[1].content, "Ê«›ﬁ «·„’—›");
			strcpy((char *)stPageContent.stContent[7].stLineContent[2].content, "»„ÊÃ»Â ⁄·Ï ‘—«¡");
			strcpy((char *)stPageContent.stContent[7].stLineContent[3].content, "«·»÷«∆⁄ „‰ﬂ„ (√Ê");

			strcpy((char *)stPageContent.stContent[8].stLineContent[0].content, "Õ’… ‘«∆⁄… „‰");
			strcpy((char *)stPageContent.stContent[8].stLineContent[1].content, "«·»÷«∆⁄ ›Ì Õ«· ");
			strcpy((char *)stPageContent.stContent[8].stLineContent[2].content, " ÕœÌœ ‰”» Â«");
			strcpy((char *)stPageContent.stContent[8].stLineContent[3].content, "›Ì„« Ì·Ì) Ê›ﬁ ");

			strcpy((char *)stPageContent.stContent[9].stLineContent[0].content, "«· ›«’Ì· «· «·Ì…: ");
			strcpy((char *)stPageContent.stContent[9].stLineContent[1].content, "‰Ê⁄ «·»÷«⁄…:† ");
			sprintf((char *)szDispBuffTmp, "%-16.16s", gPTStruct.ProductDescription);
			//PubTrimTailChars(szDispBuffTmp, ' ');
			sprintf((char *)szDispBuff, "%-16.16s", szDispBuffTmp);
			stPageContent.stContent[9].stLineContent[2].ucMode = ARABIC_CFONT;
			strcpy((char *)stPageContent.stContent[9].stLineContent[2].content, szDispBuff);


			strcpy((char *)stPageContent.stContent[10].stLineContent[0].content, "Ê’› «·»÷«⁄…:");
			//result = gPTStruct.ProductCode[0] * 10 + gPTStruct.ProductCode[1];
			//sprintf((char *)szDispBuff, "%-16d", result);
			sprintf((char *)szDispBuff, "%-16.16s", gPTStruct.product[i].proddesc);	
			//PubTrimTailChars(szDispBuff, ' ');
			stPageContent.stContent[10].stLineContent[1].ucMode = ARABIC_CFONT;
			strcpy((char *)stPageContent.stContent[10].stLineContent[1].content, szDispBuff);


			strcpy((char *)stPageContent.stContent[10].stLineContent[2].content, "ﬂ„Ì… «·»÷«⁄…:");
			sprintf((char *)szDispBuff, "%d", gPTStruct.product[i].numbers);//gPTStruct.TotItems
			stPageContent.stContent[10].stLineContent[3].ucMode = ARABIC_CFONT;
			strcpy((char *)stPageContent.stContent[10].stLineContent[3].content, szDispBuff);

			strcpy((char *)stPageContent.stContent[11].stLineContent[0].content, "—ﬁ„ —„“ «·‘—Ìÿ:");
			sprintf((char *)szDispBuffTmp, "%-16.16s", gPTStruct.product[i].barcode);		//pos screen 12
			sprintf((char *)szDispBuff, "%-16.16s", szDispBuffTmp);
			stPageContent.stContent[11].stLineContent[1].ucMode = ARABIC_CFONT;//ARABIC_ASCII;
			strcpy((char *)stPageContent.stContent[11].stLineContent[1].content, szDispBuff);
			strcpy((char *)stPageContent.stContent[11].stLineContent[2].content, "≈Ã„«·Ì «· ﬂ·›…:");
			memset(szDispBuff, 0, sizeof(szDispBuff));
			...
			FormatAmt(szDispBuff, ByDivAmout, gCurrencySymbol, ASCII_IN);
			//FormatAmt(szDispBuff, gPTStruct.EntryAmount, gCurrencySymbol, ASCII_IN);
			//=====================================================================================
			PubTrimHeadChars(szDispBuff, ' ');
			sprintf((char *)szDispBuffTmp, "%-16.16s", szDispBuff);
			sprintf((char *)szDispBuff, "%-16.16s", szDispBuffTmp);
			stPageContent.stContent[11].stLineContent[3].ucMode = ARABIC_CFONT;
			strcpy((char *)stPageContent.stContent[11].stLineContent[3].content, szDispBuff);

			strcpy((char *)stPageContent.stContent[12].stLineContent[0].content, "«·‘—«¡ ·Õ’…");
			strcpy((char *)stPageContent.stContent[12].stLineContent[1].content, "‘«∆⁄… „ﬁœ«—Â« ");
			...
			ulRet = DivStr(DivAmout, ByDivAmout, PercentAmout, 5);
			//ulRet = atol(PercentAmout);
			sprintf((char *)szDispBuff, "%s%d.%02d", "%", ulRet/100, ulRet%100);	//gPTStruct.SchemeProfit

			//sprintf((char *)szDispBuff, "%.5s%%", ascRate);
			strcpy((char *)stPageContent.stContent[12].stLineContent[2].content, szDispBuff);
			strcpy((char *)stPageContent.stContent[12].stLineContent[3].content, "„‰ «·»÷«∆⁄.");

			if (DisplayContent(13, NO_SELECT_PAGE) != STAT_OK)
			{
				return STAT_NOK;
			}
		}
		...
		memset(&stPageContent, 0, sizeof(ST_PAGECONTENT));
		//screen 27 corresponding with the english slight 22--"BY THE BANK WITHIN 10"
		strcpy((char *)stPageContent.stContent[0].stLineContent[0].content, "- ≈‰ ⁄ﬁœ «·‘—«¡");
		strcpy((char *)stPageContent.stContent[0].stLineContent[1].content, "Â–« ﬁ«»· ··≈·€«¡ ");
		strcpy((char *)stPageContent.stContent[0].stLineContent[2].content, "„‰ ÿ—› «·„’—›");
		strcpy((char *)stPageContent.stContent[0].stLineContent[3].content, "Œ·«· 10 √Ì«„ „‰");

		strcpy((char *)stPageContent.stContent[1].stLineContent[0].content, " «—ÌŒ ⁄ﬁœ «·‘—«¡.  ");
		strcpy((char *)stPageContent.stContent[1].stLineContent[1].content, "-  ŒÊ· «·‰”Œ ");
		strcpy((char *)stPageContent.stContent[1].stLineContent[2].content, "«·„ÿ»Ê⁄… „‰");
		strcpy((char *)stPageContent.stContent[1].stLineContent[3].content, "√ÃÂ“… ‰ﬁ«ÿ «·»Ì⁄ ");

		strcpy((char *)stPageContent.stContent[2].stLineContent[0].content, "«·Œ«’… »⁄ﬁœ");
		strcpy((char *)stPageContent.stContent[2].stLineContent[1].content, "«·‘—«¡ „„À·");
		strcpy((char *)stPageContent.stContent[2].stLineContent[2].content, "«·„’—› √‰ Ì ”·¯„");
		strcpy((char *)stPageContent.stContent[2].stLineContent[3].content, "«·»÷«⁄… «·„‘ —«…");

		strcpy((char *)stPageContent.stContent[3].stLineContent[0].content, "(»«·‰Ì«»… ⁄‰");
		strcpy((char *)stPageContent.stContent[3].stLineContent[1].content, "«·„’—›)  ”·„« ");
		strcpy((char *)stPageContent.stContent[3].stLineContent[2].content, "ÕﬁÌﬁÌ« Ê«· ’—› ");
		strcpy((char *)stPageContent.stContent[3].stLineContent[3].content, "Ê›ﬁ „·Õﬁ  ÊﬂÌ·Â");

		strcpy((char *)stPageContent.stContent[4].stLineContent[0].content, "œÊ‰ √Ì „«‰⁄ „‰");
		strcpy((char *)stPageContent.stContent[4].stLineContent[1].content, "ÿ—› «· «Ã— ”Ê«¡");
		strcpy((char *)stPageContent.stContent[4].stLineContent[2].content, " „  ⁄„·Ì… «·»Ì⁄ ");
		strcpy((char *)stPageContent.stContent[4].stLineContent[3].content, "·„ ⁄«„· «· „ÊÌ· ");

		strcpy((char *)stPageContent.stContent[5].stLineContent[0].content, "«·›Ê—Ì √„ ·„   „.");
		strcpy((char *)stPageContent.stContent[5].stLineContent[2].content, "⁄“Ì“Ì «· «Ã—.");
		strcpy((char *)stPageContent.stContent[5].stLineContent[3].content, "Ì—ÃÏ «·÷€ÿ ⁄·Ï ");

		strcpy((char *)stPageContent.stContent[6].stLineContent[0].content, "√Õœ «·ŒÌ«—« ");
		strcpy((char *)stPageContent.stContent[6].stLineContent[1].content, "«· «·Ì…: ");

		strcpy((char *)stPageContent.stContent[7].stLineContent[0].content, "1-ﬁ»·  «·»Ì⁄ ");
		strcpy((char *)stPageContent.stContent[7].stLineContent[1].content, "··„’—›.");
		strcpy((char *)stPageContent.stContent[7].stLineContent[2].content, "2-·„ √ﬁ»· «·»Ì⁄ ");
		strcpy((char *)stPageContent.stContent[7].stLineContent[3].content, "··„’—›");

		if (DisplayContent(8, 7) == KEY1)
		{
			FileInit(FILE_RT);		// Added by Kim_LinHB 2014-4-11
			Print_SaleDraft_Slip();				//printout 1
			//Print_SaleContractSub(1);
...
// 		//screen 19
// 		ArShowString(0, SHIFT_RIGHT, "⁄“Ì“?«· «Ã—.."); //pos sreen1


// 		//screen 20

// 		ScrCls();
// 		ArShowString(0, SHIFT_RIGHT, "„ ⁄«„· «· „ÊÌ?");//pos screen 3


// 		ScrCls();
// 		ArShowString(0, SHIFT_RIGHT, "«· „ÊÌ?«·›Ê—Ì");//pos screen 4

// 
// 		//screen 21

// 		ScrCls();
// 		ArShowString(0, SHIFT_RIGHT, "„‰  ﬂ·› Â?›ﬁ?");//pos screen 5



// 		//screen 22
// 		ArShowString(0, SHIFT_RIGHT, "«·»÷«∆? ");	//pos screen 7
// 		//screen 23

// 		ScrCls();			//pos screen 8
// 		ScrCls();
// 		ArShowString(0, SHIFT_RIGHT, "Õ’?‘«∆⁄?„‰");//pos screen 9	


// 				sprintf(szDispBuff, "%-13.13s", gPTStruct.product[i].barcode);//pos screen 12
...
// 		ArShowString(0, SHIFT_RIGHT, " «—Ì?⁄ﬁ?«·‘—«¡.  ");//pos screen 15
// 		//screen 28

// 		ArShowString(0, SHIFT_RIGHT, "(»«·‰Ì«»… ⁄‰");	//pos screen 17

// 		//screen 29

// 		ArShowString(6, SHIFT_RIGHT, "·„ ⁄«„?«· „ÊÌ?");//pos screen 18

// 		//screen 26
// 		ArShowString(0, SHIFT_RIGHT, "1- ﬁ»·  «·»Ì?"); //pos screen 21
	}
	else
	{
// 		//screen 19
// 		if (ARABIC != gLanguage)
// 		{	
// 			strcpy(MainMSG.CSTMSG1, "DEAR MERCHANT,");
// 			strcpy(MainMSG.CSTMSG2, "ADIB HEREBY AGREES TO");
// 			strcpy(MainMSG.CSTMSG3, "BUY FROM YOU THE");
// 			strcpy(MainMSG.CSTMSG4, "ENTIRE FOLLOWING");
// 			strcpy(MainMSG.CSTMSG5, "PRODUCTS (OR A COMMON");
// 			strcpy(MainMSG.CSTMSG6, "SHARE OF THE PRODUCTS");
// 		}
// 
// 		if (stMSG->CSTMSG1[0])	// If Line1 exists
// 	    {
...
// 		}
		...
		{
			memset(&stPageContent, 0, sizeof(ST_PAGECONTENT));
			strcpy((char *)stPageContent.stContent[0].stLineContent[0].content, "DEAR MERCHANT,");
			strcpy((char *)stPageContent.stContent[0].stLineContent[1].content, "ADIB HEREBY AGREES TO");
			strcpy((char *)stPageContent.stContent[0].stLineContent[2].content, "BUY FROM YOU THE");
			strcpy((char *)stPageContent.stContent[0].stLineContent[3].content, "ENTIRE FOLLOWING");
			strcpy((char *)stPageContent.stContent[0].stLineContent[4].content, "PRODUCTS (OR A COMMON");
			strcpy((char *)stPageContent.stContent[0].stLineContent[5].content, "SHARE OF THE PRODUCTS");

			strcpy((char *)stPageContent.stContent[1].stLineContent[0].content, "IF SPECIFIED BELOW):");
			strcpy((char *)stPageContent.stContent[1].stLineContent[1].content, "PRODUCT TYPE:");
			//result = gPTStruct.ProductCode[0] * 10 + gPTStruct.ProductCode[1];
			//sprintf((char *)szDispBuff, "%21d", result);

			sprintf(szDispBuff, "%16.16s", gPTStruct.ProductDescription);
			//PubTrimHeadChars(szDispBuff, ' ');
			PubTrimTailChars(szDispBuff, ' ');
			sprintf((char *)szDispBuffTmp, "%21.21s", szDispBuff);
			strcpy((char *)stPageContent.stContent[1].stLineContent[2].content, szDispBuffTmp);

			strcpy((char *)stPageContent.stContent[1].stLineContent[3].content, "DESCRIPTION:");
// 			sprintf(szDispBuff, "%-16.16s", gPTStruct.ProductDescription);
// 			PubTrimTailChars(szDispBuff, ' ');
// 			sprintf((char *)szDispBuffTmp, "%21.21s", szDispBuff);			
			sprintf((char *)szDispBuffTmp, "%16.16s", gPTStruct.product[i].proddesc);	
			PubTrimTailChars(szDispBuffTmp, ' ');
			sprintf((char *)szDispBuff, "%21.21s", szDispBuffTmp);
			strcpy((char *)stPageContent.stContent[1].stLineContent[4].content, szDispBuff);

			strcpy((char *)stPageContent.stContent[1].stLineContent[5].content, "SHARE OF THE PRODUCTS");
			strcpy((char *)stPageContent.stContent[1].stLineContent[6].content, "QUANTITY:");
			sprintf((char *)szDispBuff, "%21d", gPTStruct.product[i].numbers);//gPTStruct.TotItems
//			sprintf((char *)szDispBuff, "%21.21s", szDispBuff);
			strcpy((char *)stPageContent.stContent[1].stLineContent[7].content, szDispBuff);

			strcpy((char *)stPageContent.stContent[2].stLineContent[0].content, "BARCODE NUMBER:");
			sprintf((char *)szDispBuff, "%16.16s", gPTStruct.product[i].barcode);
			sprintf((char *)szDispBuffTmp, "%21.21s", szDispBuff);
			strcpy((char *)stPageContent.stContent[2].stLineContent[1].content, szDispBuffTmp);
			strcpy((char *)stPageContent.stContent[2].stLineContent[2].content, "PRODUCTS' TOTAL");
			strcpy((char *)stPageContent.stContent[2].stLineContent[3].content, "COST PRICE:");
			memset(szDispBuff, 0, sizeof(szDispBuff));
			...
			PubTrimHeadChars(szDispBuff, ' ');
			sprintf((char *)szDispBuffTmp, "%21.21s", szDispBuff); 
			strcpy((char *)stPageContent.stContent[2].stLineContent[4].content, szDispBuffTmp);
			strcpy((char *)stPageContent.stContent[2].stLineContent[5].content, "THE PURCHASED SHARE");
...
// 				ascRate[2] ='.';
// 			}
// 			sprintf((char *)szDispBuff, "IS %.5s%% OF THE ENTIRE", ascRate);//gPTStruct.SchemeProfit
			...
			ulRet = DivStr(DivAmout, ByDivAmout, PercentAmout, 5);
			//ulRet = atol(PercentAmout);
			sprintf((char *)szDispBuff, "IS %d.%02d%s OF", ulRet/100, ulRet%100, "%");	//gPTStruct.SchemeProfit	 
			strcpy((char *)stPageContent.stContent[2].stLineContent[6].content, szDispBuff);
			strcpy((char *)stPageContent.stContent[2].stLineContent[7].content, "THE ENTIRE PRODUCTS");
			if (DisplayContent(3, NO_SELECT_PAGE) != STAT_OK)
			{
				return STAT_NOK;
			}
...
// 			//screen 19
		...
		memset(&stPageContent, 0, sizeof(ST_PAGECONTENT));
		strcpy((char *)stPageContent.stContent[0].stLineContent[0].content, "THIS PURCHASE");
		strcpy((char *)stPageContent.stContent[0].stLineContent[1].content, "CONTRACT IS REVOCABLE");
		strcpy((char *)stPageContent.stContent[0].stLineContent[2].content, "BY THE BANK WITHIN 10");
		strcpy((char *)stPageContent.stContent[0].stLineContent[3].content, "DAYS FROM THE");
		strcpy((char *)stPageContent.stContent[0].stLineContent[4].content, "PURCHASED CONTRACT'S");
		strcpy((char *)stPageContent.stContent[0].stLineContent[5].content, "DATE. ");
		strcpy((char *)stPageContent.stContent[0].stLineContent[7].content, "THE PRINTOUT OF THE");

		strcpy((char *)stPageContent.stContent[1].stLineContent[0].content, "PURCHASE CONTRACT");
		strcpy((char *)stPageContent.stContent[1].stLineContent[1].content, "ENTITLES ADIB");
		strcpy((char *)stPageContent.stContent[1].stLineContent[2].content, "REPRESENTATIVE TO");
		strcpy((char *)stPageContent.stContent[1].stLineContent[3].content, "PHYSICALLY RECEIVE");
		strcpy((char *)stPageContent.stContent[1].stLineContent[4].content, "THE PURCHASED PRODUCTS");
		strcpy((char *)stPageContent.stContent[1].stLineContent[5].content, "(ON BEHALF OF ADIB)");
		strcpy((char *)stPageContent.stContent[1].stLineContent[6].content, "AND ENABLES HIM/HER");
		strcpy((char *)stPageContent.stContent[1].stLineContent[7].content, "TO PERFORM HIS/HER");

		strcpy((char *)stPageContent.stContent[2].stLineContent[0].content, "DUTIES THAT ARE");
		strcpy((char *)stPageContent.stContent[2].stLineContent[1].content, "SPECIFIED IN THE");
		strcpy((char *)stPageContent.stContent[2].stLineContent[2].content, "RELEVANT SCHEDULES");
		strcpy((char *)stPageContent.stContent[2].stLineContent[3].content, "WITHOUT ANY OBJECTION");
		strcpy((char *)stPageContent.stContent[2].stLineContent[4].content, "FROM THE MERCHANT");
		strcpy((char *)stPageContent.stContent[2].stLineContent[5].content, "SIDE REGARDLESS OF");
		strcpy((char *)stPageContent.stContent[2].stLineContent[6].content, "SELLING THE SAME TO");
		strcpy((char *)stPageContent.stContent[2].stLineContent[7].content, "THE INSTANT FINANCE");

		strcpy((char *)stPageContent.stContent[3].stLineContent[0].content, "CUSTOMER OR NOT.");

		strcpy((char *)stPageContent.stContent[3].stLineContent[2].content, "DEAR MERCHANT");
		strcpy((char *)stPageContent.stContent[3].stLineContent[3].content, "PLEASE SELECT ONE OF");
		strcpy((char *)stPageContent.stContent[3].stLineContent[4].content, "THE FOLLOWING:");
		strcpy((char *)stPageContent.stContent[3].stLineContent[5].content, "1- ACCEPTED TO SELL");
		strcpy((char *)stPageContent.stContent[3].stLineContent[6].content, " TO ADIB");
		strcpy((char *)stPageContent.stContent[3].stLineContent[7].content, "2- REJECTED TO SELL");

		strcpy((char *)stPageContent.stContent[4].stLineContent[0].content, "TO ADIB?");

		if (DisplayContent(5, 3) == KEY1)
		{
			FileInit(FILE_RT);		// Added by Kim_LinHB 2014-4-11
			Print_SaleDraft_Slip();				//printout 1
			//Print_SaleContractSub(1);
...
// 		//screen22
// 		ScrCls();
// 		ScrPrint(0, 0, ASCII, "THIS PURCHASE");
// 		ScrPrint(0, 1, ASCII, "CONTRACT IS REVOCABLE");
// 		ScrPrint(0, 2, ASCII, "BY THE BANK WITHIN 10");
// 		ScrPrint(0, 3, ASCII, "DAYS FROM THE");
// 		ScrPrint(0, 4, ASCII, "PURCHASED CONTRACT'S");
// 		ScrPrint(0, 5, ASCII, "DATE. ");
// 		ScrPrint(0, 7, ASCII, "THE PRINTOUT OF THE");
// 		
// 		while (1)
// 		{
// 			ucKey = GetOneKey(TO_1MIN);
...
// 		ScrCls();
// 		ScrPrint(0, 0, ASCII, "PURCHASE CONTRACT");
// 		ScrPrint(0, 1, ASCII, "ENTITLES ADIB");
// 		ScrPrint(0, 2, ASCII, "REPRESENTATIVE TO");
// 		ScrPrint(0, 3, ASCII, "PHYSICALLY RECEIVE");
// 		ScrPrint(0, 4, ASCII, "THE PURCHASED PRODUCTS");
// 		ScrPrint(0, 5, ASCII, "(ON BEHALF OF ADIB)");
// 		ScrPrint(0, 6, ASCII, "AND ENABLES HIM/HER");
// 		ScrPrint(0, 7, ASCII, "TO PERFORM HIS/HER");
// 		
// 		while (1)
// 		{
// 			ucKey = GetOneKey(TO_1MIN);
...
// 		//screen 23
// 		ScrCls();
// 		ScrPrint(0, 0, ASCII, "DUTIES THAT ARE");
// 		ScrPrint(0, 1, ASCII, "SPECIFIED IN THE");
// 		ScrPrint(0, 2, ASCII, "RELEVANT SCHEDULES");
// 		ScrPrint(0, 3, ASCII, "WITHOUT ANY OBJECTION");
// 		ScrPrint(0, 4, ASCII, "FROM THE MERCHANT");
// 		ScrPrint(0, 5, ASCII, "SIDE REGARDLESS OF");
// 		ScrPrint(0, 6, ASCII, "SELLING THE SAME TO");
// 		ScrPrint(0, 7, ASCII, "THE I.F.C OR NOT. ");
// 		
// 		while (1)
// 		{
// 			ucKey = GetOneKey(TO_1MIN);
...
// 		//screen 26
// 		ScrCls();
// 		ScrPrint(0, 0, ASCII, "DEAR MERCHANT");
// 		ScrPrint(0, 1, ASCII, "PLEASE SELECT ONE OF");
// 		ScrPrint(0, 2, ASCII, "THE FOLLOWING:");
// 		ScrPrint(0, 4, ASCII, "1- ACCEPTED TO SELL");
// 		ScrPrint(0, 5, ASCII, " TO ADIB");
// 		ScrPrint(0, 6, ASCII, "2- REJECTED TO SELL");
// 		ScrPrint(0, 7, ASCII, "TO ADIB?");

	}

	//Type is always equal to 1
// 	if (!Type) 
// 	{
// 		return (1);
// 	}
