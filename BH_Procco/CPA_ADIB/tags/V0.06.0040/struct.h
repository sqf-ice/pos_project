...
#define LG_PRODUCT_TYPE 25
	BYTE	prodinfo[21 + 1];
	BYTE    barcode[16+1];	//ADIB maximum length of barcode changes into 16. it's 13 before the version of ADIB
...
//	BYTE Balance[LG_AMOUNT];           //12
	...
	BYTE AvailableInstallmentFlag1;    //1
	BYTE AvailableInstallmentFlag2;    //1
	BYTE EntryAmount[LG_AMOUNT];       //12		Principal Payment = unit price * unit number
	BYTE AdvancedAmount[LG_AMOUNT];    //12		advanced amount --> InputAmountExt()
	BYTE TotalInstallmentCount;        //1
	BYTE MonthlyInstAmount[LG_AMOUNT]; //12
	BYTE TotalAmount[LG_AMOUNT];       //12
	BYTE ProfitAmountBcd[6];           //6
...
///////////////////////////////////////////////////////////////////////////////
	BYTE ProductDescription[16+1];       //12		12-->16
...
#define NUM_PRODUCTS_PERTRAN    12
    PRODUCT product[NUM_PRODUCTS_PERTRAN];
	
	BYTE TotItems;						//-->for test
//	BYTE ProdCode[2];					
	BYTE MaxTenure;						
	BYTE HMinTenure;					
	BYTE HMaxTenure;
