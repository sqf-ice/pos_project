...
BYTE StartBatchClose(void)
{
    ...
    else
    {
		...
		FileCopy(FILE_LAST_EOD, FILE_TR);     // add by Allen 20151116
...
//    BYTE lpDate[13], lpTime[13];
    ...
    int  Count=0;
//by jarod 20110831
//ABD said, only print the last line
/*
    if (HDGRead(&lsHDGBuf)!=STAT_OK)
    {
        return(STAT_INV);
