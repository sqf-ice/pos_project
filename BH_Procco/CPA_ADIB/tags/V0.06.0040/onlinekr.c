...
//ACCONT MAINT-----------NET SERVER
	{ {0x60},{0x06,0x61},{0x00,0x00},{0x01,0x00},{0x91,0x00,0x00},{0x72,0x24,0x06,0x80,0x20,0x80,0x82,0x02} },// ACCOUNT MAINT
//ACCONT MAINT ADVICE----NET SERVER
	{ {0x60},{0x00,0x03},{0x00,0x00},{0x01,0x20},{0x91,0x00,0x00},{0x72,0x24,0x06,0x80,0x08,0x80,0x82,0x02} },// ACCOUNT MAINT ADVICE
//REVERSAL---------------NET SERVER
...
//INSTALLMENT ADVICE-----PMS
	{ {0x60},{0x00,0x05},{0x00,0x00},{0x02,0x20},{0x00,0x00,0x00},{0x70,0x3C,0x06,0x80,0x0A,0x82,0x82,0x00} },// INSTALLMENT ADVICE
//LOYALTY ADVICE---------PMS
	{ {0x60},{0x00,0x05},{0x00,0x00},{0x02,0x20},{0x16,0x00,0x00},{0x70,0x3C,0x06,0x80,0x08,0x82,0x80,0x00} },// LOYALTY ADVICE
...
//BALANCE
#ifdef ADIB_CHANGES											
	{ {0x60},{0x06,0x61},{0x00,0x00},{0x01,0x00},{0x30,0x00,0x00},{0x72,0x2C,0x06,0x80,0x20,0x82,0x80,0x02} },
#else
	{ {0x60},{0x06,0x61},{0x00,0x00},{0x01,0x00},{0x30,0x00,0x00},{0x72,0x2C,0x06,0x80,0x20,0x80,0x80,0x02} },// BALANCE INQ
#endif
//RETAIL
	{ {0x60},{0x06,0x61},{0x00,0x00},{0x02,0x00},{0x00,0x00,0x00},{0x72,0x24,0x06,0x80,0x20,0x82,0x82,0x02} },// 
...
static char DisPlayErrMsgInF47(unsigned char Type);
// Add End

//by jarod 2011.06.01
void ISO_DisPlayErrMsg(byte*Field_Attr)
{
	unsigned int Recp_Code;
	int i;
	int nDefErrFlag;
	// Added by Kim_LinHB 2014-3-11
	if(!Field_Attr)
		return;
...
// 	Recp_Code-=48;
	...
	Recp_Code = (Field_Attr[0] - 0x30) * 10 + (Field_Attr[1] - 0x30);
	if(0 == Recp_Code)
		return;
	// Modified by Kim_LinHB 2014-3-7 from 10 to real range
	for(i=0; gHostErrMsgTableFlag[i]>0;i++)
	{
		nDefErrFlag = i;
		if(gHostErrMsgTableFlag[i]==Recp_Code)
		{
			Print_HostErr_Slip((char *)gHostErrMsgForPRT[i]);
			DisplayLongString(gHostErrMsgForDisp[i],2000*100);
			ScrCls();
			return;
		}
	}
	// print the default info
	Print_HostErr_Slip((char *)gHostErrMsgForPRT[nDefErrFlag]);
	DisplayLongString(gHostErrMsgForDisp[nDefErrFlag],2000*100);
	ScrCls();
}
...
//    int  i;
	...
	int nErrorCode = STAT_OK; // Added by Kim_LinHB 2014-3-11
	...
	memset(&ISOData, 0, sizeof(ISODataStruct));
    ///////////////////////////////////////////////////
    ///Card Number
    ///////////////////////////////////////////////////
	...
	if (Type == ISO_GETBALANCE || 
		Type == ISO_ACCOUNTMAINTENANCE || // Added by Kim_LinHB 2014-4-17
	...
	if(/*(Type==ISO_PAYMENT) ||*/
	   ...
	   || (Type==ISO_LOYALTY_ADVICE)
// 	   || (Type==ISO_ACCOUNTMAINTENANCE)  // Removed by Kim_LinHB 2014-4-17
// 	   || (Type==ISO_ACCOUNTMAINTENANCE_ADVICE)
	   || (Type==ISO_GETBALANCE)
	   /*|| (Type==ISO_RETAIL)*/)
    {
	...
	else if ( (Type==ISO_PAYMENT) || (Type==ISO_RETAIL))
	{
		memcpy(ISOData.Field22,"\x00\x51",2);	//for ADIB_CHANGES in ISO_PAYMENT/ISO_RETAIL
	}
	if((Type==ISO_INSTALLMENT_ADVICES)||
    ...
    memcpy(ISOData.Field35+1,TmpStr, gPTStruct.Tr2Len);
...
// 		/*!!!sel???
// 		memcpy(ISOData.Field47+lbCount,"\x00\x41", 2);                   lbCount=lbCount+2;
// 		memcpy(ISOData.Field47+lbCount,"\xE2\x00", 2);                   lbCount=lbCount+2;
// 		*/
// ///////////////////////////////////////////////////////////////////////////////////		
// 		memcpy(CurrF47+lbCount,"\xC1\x04\x54", 3);               lbCount=lbCount+3;
// 		memcpy(CurrF47+lbCount,"POS", 3);          lbCount=lbCount+3;
// ///////////////////////////////////////////////////////////////////////////////////		
// 		/*!!!not required host will calculate  
// 		memcpy(ISOData.Field47+lbCount,"\xC2\x07\x46", 3);               lbCount=lbCount+3;
// 		memcpy(lbTmpTotal,gPTStruct.MonthlyInstAmount,LG_AMOUNT);
// 		lbTmpTotal[LG_AMOUNT] = 0;
// 		StrTrim(lbTmpTotal,lbTmpTotal1,' ');
// 		StrLeftPad(lbTmpTotal1,LG_AMOUNT,lbTmpTotal1,'0');
// 		lbTmpTotal1[LG_AMOUNT]=0;
// 		Utils_ConvertAscToBcd(lbTmpTotal1, Bcd, 12, 0, PAD_NONE,0);
// 		memcpy(gPTStruct.BcdAmount, Bcd, 6);
// 
...
// 		*/
// ///////////////////////////////////////////////////////////////////////////////////		
// // 		memcpy(CurrF47+lbCount,"\xC3\x03\x46", 3);               lbCount=lbCount+3;
// // 		/*StrBcdToAsc(4, gPTStruct.CurrencyCode,lbTmpTotal);
// // 		memcpy(lbTmpTotal1,lbTmpTotal+1, 3); 
// // 		lbTmpTotal1[3]='F';
// // 		lbTmpTotal1[4]=0;
...
// // 		AscToHex((PCHAR)lbTmpTotal1, (PCHAR)lbTmpTotal,2);*/
// // 		
// // 		memcpy(CurrF47+lbCount,gPTStruct.CurrencyCode, 2);       lbCount=lbCount+2;
// 
// 		memcpy(ISOData.Field47+lbCount,"\xC3\x03\x46", 3);               lbCount=lbCount+3;
// 		StrBcdToAsc(4, gPTStruct.CurrencyCode,lbTmpTotal);
// 		memcpy(lbTmpTotal1,lbTmpTotal+1, 3); 
...
//  		memcpy(ISOData.Field47+lbCount,lbTmpTotal, 2);					 lbCount=lbCount+2;
// ///////////////////////////////////////////////////////////////////////////////////		
// 		memcpy(CurrF47+lbCount,"\xC4\x02\x44", 3);               lbCount=lbCount+3;
// 
// 		lbTmpTotal[0]=(gPTStruct.TotalInstallmentCount/10)+'0';
// 		lbTmpTotal[1]=(gPTStruct.TotalInstallmentCount%10)+'0';lbTmpTotal[2]=0;
// 		Utils_ConvertAscToBcd(lbTmpTotal, lbTmpTotal1, 2, 0, PAD_NONE,0);
// 		CurrF47[lbCount]=lbTmpTotal1[0]; lbCount=lbCount+1;
// ///////////////////////////////////////////////////////////////////////////////////		
// 		memcpy(CurrF47+lbCount,"\xC6\x07\x46", 3);               lbCount=lbCount+3;
// 		
// 		memcpy(CurrF47+lbCount,gPTStruct.ProfitAmountBcd, 6);    lbCount=lbCount+6;
// // 		for (i=0;i<gPTStruct.TotItems;i++) {		
// // 			sprintf(TmpF47+strlen(TmpF47),"#%s,%s,%s,%d,%.12s",
// // 				gPTStruct.prodinfo[i],
// // 				gPTStruct.proddesc[i],
// // 				gPTStruct.barcode[i],
// // 				gPTStruct.numitems[i],
// // 				gPTStruct.ItemAmount[i]);
// // 		}
// 		for (i=0;i<gPTStruct.TotItems;i++)
// 		{	
// 			sprintf(TmpF47,"#%s,%s,%s,%d,%.6s", 
// 				gPTStruct.product[i].prodinfo, 
// 				gPTStruct.product[i].proddesc, 
// 				gPTStruct.product[i].barcode, 
// 				gPTStruct.product[i].numbers, 
// 				gPTStruct.product[i].price_bcd);
// 		}
// 		CurrF47[lbCount]=0xC7;
// 		lbCount++;	
// 		if ((strlen(TmpF47)+1)>127) {
// 			CurrF47[lbCount]=0x81;
// 			lbCount++;
// 		}
// 		CurrF47[lbCount]=strlen(TmpF47)+1;
// 		lbCount++;
// 		memcpy(CurrF47+lbCount,"\x54", 1);   
// 		lbCount++;	
// 		memcpy(CurrF47+lbCount,TmpF47,strlen(TmpF47));   lbCount=lbCount+strlen(TmpF47);
// 		memcpy(CurrF47+lbCount,"\xCA\x04\x54", 3);               lbCount=lbCount+3;
// 		memcpy(lbTmpTotal,gPTStruct.ProductCode, 2);
// 		lbTmpTotal[2]=0;
// 		StrBcdToAsc(4,lbTmpTotal,lbTmpTotal1);
// 		memcpy(CurrF47+lbCount,lbTmpTotal1+1, 3); lbCount=lbCount+3;
// 		//memcpy(ISOData.Field47+lbCount,"\x30\x30\x35", 3); lbCount=lbCount+3;
// #ifdef APPL_ALBARAKA	
// 		memcpy(CurrF47+lbCount,"\xC8\x07\x46", 3);               lbCount=lbCount+3;
// 		memcpy(lbTmpTotal,gPTStruct.Insurance,LG_AMOUNT);
// 		
// 		lbTmpTotal[LG_AMOUNT]=0;
// 		Utils_ConvertAscToBcd(lbTmpTotal, Bcd, 12, 0, PAD_NONE,0);
// 		memcpy(CurrF47+lbCount,Bcd, 6);          lbCount=lbCount+6;
// #else
// 		memcpy(CurrF47+lbCount,"\xC8\x07\x46", 3);               lbCount=lbCount+3;
// 		memset(Bcd,0,6);
// 		memcpy(CurrF47+lbCount,Bcd, 6);          lbCount=lbCount+6;
...
// 
// 		
// 		memcpy(ISOData.Field47,"\x00\x00\xE2", 3);                   
// 		if (lbCount>127) {
// 			ISOData.Field47[3]=0x81;
// 			ISOData.Field47[4]=lbCount;
// 			memcpy(ISOData.Field47+5, CurrF47, lbCount);
// 			lbCount+=3;
// 		}
// 		else {
// 			ISOData.Field47[3]=lbCount;
// 			lbCount+=2;
// 			memcpy(ISOData.Field47+4,CurrF47,lbCount);
// 		}	
// 		sprintf(Bcd,"%04d",lbCount);
// 		Utils_ConvertAscToBcd(Bcd, ISOData.Field47, 4, 0, PAD_NONE,0);	
// 		lbCount+=2;
	}
#ifdef ADIB_CHANGES	
	else if(Type==ISO_GETBALANCE) 
	{
		...
		memcpy(ISOData.Field47+lbCount, "\xC8\x07\x46", 3);              lbCount=lbCount+3;
		// Modified by Kim_LinHB 2014-3-13
		if(memcmp(gPTStruct.EntryAmount, gPTStruct.AdvancedAmount, LG_AMOUNT) >= 0)
		{
			PubAscSub(gPTStruct.EntryAmount, gPTStruct.AdvancedAmount, LG_AMOUNT, lbTmpTotal);
		}
		else
		{
			PubAscSub(gPTStruct.AdvancedAmount, gPTStruct.EntryAmount, LG_AMOUNT, lbTmpTotal);
			lbTmpTotal[0] = '-';
		}
		// Add End
...
//		memcpy(lbTmpTotal,gPTStruct.EntryAmount,LG_AMOUNT);		
		...
		memcpy(ISOData.Field47+lbCount,"\xCA\x04\x54", 3); lbCount=lbCount+3;
//		memcpy(lbTmpTotal,gPTStruct.ProdCode, 2);
		memcpy(lbTmpTotal,gPTStruct.ProductCode, 2);
		lbTmpTotal[2]=0;
		StrBcdToAsc(4,lbTmpTotal,lbTmpTotal1);
		memcpy(ISOData.Field47+lbCount,lbTmpTotal1+1, 3); lbCount=lbCount+3;
...
//	memcpy(ISOData.Field49,"\x00\x48",2);
    ...
    ///Send / Receive
    ////////////////////////////////////////////////////
	nErrorCode = ISO_Transaction(Type);
	if(nErrorCode==ISO_STAT_OK)
	{
        /////////////////////////////////////////////
        /////////////////////////////////////////////
		if((Type == ISO_BATCH_UPLOAD_ADVICE)||
		...
		else
		{
			...
			ClkUpdateDateTime(gPTStruct.BcdDateTime);						//for test warning
...
//MK 02112007 Lost and Stolen Card
				if(FALSE == DisPlayErrMsgInF47(Type))
			...
			{
				if(Type==ISO_RETAIL || Type == ISO_ACCOUNTMAINTENANCE||
					Type == ISO_ACCOUNTMAINTENANCE_ADVICE)//add by chens 2014.04.22
				{
					if(ScriptParser()!=STAT_OK)
					{
						DisplayString((PCHAR)"SCRIPT ERROR",2000);
				...
				{
					// Added by Kim_LinHB 2014-3-7 check DB print DC
					if(TRUE == DisPlayErrMsgInF47(Type))
					{
						return (STAT_INV);
					}
					// Add End
...
{
    //ISOFrame frame;
    unsigned char retVal = ISO_STAT_OK; // Modified by Kim_LinHB 2014-5-29
    int      ISOPacketLength;
...
/* 
return	TRUE	: printed 
		FALSE	: did not printed
*/
static char DisPlayErrMsgInF47(unsigned char Type)
{
	unsigned int len47, lenF0, lenTag;
	BYTE *F0, *Tag;
	BYTE Value[255], szErrMsg[255] = "ERROR:\n"; 
	...
	F0 = FindStr(ISOData.Field47 + 2, len47, "\xF0", 1);
	if (NULL == F0)
	{
		return FALSE;
	}
	...
	ScrCls();
	DisplayLongString(szErrMsg,2000*100);
	Print_HostErr_Slip(szErrMsg);
	ScrCls();
...
{
	unsigned int length;
	BYTE out[15],Offset=2,tag,taglen=0;
	if (ISOData.Field47[Offset]!=0xF0)
		return;
	Offset++;
	length=ISOData.Field47[Offset];
	Offset++;
	length+=4;
	while (Offset<length)
	{
		tag=ISOData.Field47[Offset];
		
		switch (tag)
		{
		case 0xD1:
			Offset++;
			taglen=ISOData.Field47[Offset];
			taglen--;
			Offset+=2;
			memcpy(out, ISOData.Field47+Offset, taglen);					//yearly profit rate
			out[taglen] = 0;
			Utils_ConvertAscToBcd(out, gPTStruct.SchemeProfit, 4, 0, PAD_NONE,0);
			//gPTStruct.SchemeProfit
			Offset+=taglen;
		...
		case 0xD2:
			...
			memset(gPTStruct.HMinTotal,0x30,LG_AMOUNT);
			memcpy(gPTStruct.HMinTotal+(LG_AMOUNT-taglen),ISOData.Field47+Offset,taglen);
			Offset+=taglen;
		...
		case 0xD3:
			...
			memset(gPTStruct.HMaxTotal,0x30,LG_AMOUNT);
			memcpy(gPTStruct.HMaxTotal+(LG_AMOUNT-taglen),ISOData.Field47+Offset,taglen);
			Offset+=taglen;
		...
		case 0xD4:
			...
			memcpy(out, ISOData.Field47+Offset,taglen);
			out[taglen]=0;
			gPTStruct.HMinTenure = atoi((char *)out);
			Offset+=taglen;
		...
		case 0xD5:
			...
			memcpy(out, ISOData.Field47+Offset,taglen);
			out[taglen]=0;
			gPTStruct.HMaxTenure = atoi((char *)out);
			Offset+=taglen;
...
// 			memcpy(out, ISOData.Field47+Offset,taglen);
// 			out[taglen]=0;
// 			gPTStruct.AskEntries = atoi((const char *)out);
// 			Offset+=taglen;
// 			break;
		case 0xD6:
			Offset++;
			...
			memcpy(out, ISOData.Field47+Offset,taglen);
			out[taglen]=0;
			gPTStruct.lTenure = (unsigned long)atol((char *)out);
			Offset+=taglen;
			break;
 //			gPTStruct.AskEntries = atoi((const char *)out);
// 			Offset+=taglen;
