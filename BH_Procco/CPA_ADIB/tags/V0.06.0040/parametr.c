...
WORD StartParameterDload(BYTE fAutoParam )
{
	...
	EMV_CAPK    stPaxCAPK;			//for test20121212
#endif
    ...
    if (lpWrkBuf[2] == 'R')
    {
        ...
        if (Delete(FILE_PE)!=0)
            return (STAT_INV);
        if (Rename(FILE_TP,FILE_PE)!=0)//Tenure
            return (STAT_INV);
        if (HDGRead(&lsHDGBuf)!=STAT_OK)
            return(STAT_NOK);
    ...
    if (lpWrkBuf[2] == 'K')
    {
        ...
        if (lwStatus != STAT_OK)
            return(lwStatus);
#ifdef DEVELOP_VERSION
		for (lwStatus = 0; lwStatus < 64; lwStatus++)
		{
			EMVGetCAPK(lwStatus, &stPaxCAPK);
			ScrCls();
			ScrPrint(0, 0, CFONT, "%02x %02x %02x %02x %02x",
				stPaxCAPK.RID[0], stPaxCAPK.RID[1], stPaxCAPK.RID[2], stPaxCAPK.RID[3], stPaxCAPK.RID[4]);
			ScrPrint(0, 2, CFONT, "%02x Ex:%02x %02x %02x", stPaxCAPK.KeyID, stPaxCAPK.Exponent[0], stPaxCAPK.Exponent[1], stPaxCAPK.Exponent[2]);
			ScrPrint(0, 4, CFONT, "%02x %02x %02x", stPaxCAPK.ExpDate[0], stPaxCAPK.ExpDate[1], stPaxCAPK.ExpDate[2]);
			getkey();
		}
#endif
    ...
    *fpBlockNo=fpWrkBuf[1]-'0';
    ...
    {                  
        ...
        */
        ...
        HexToAsc(lsKeyInfoBuf.keyLocationPointer, (BYTE *)tmpbuf, (BYTE)lsKeyInfoBuf.keyLength);//for test warning
