...
static s_display_barcode(BYTE *pszBarcode)
{
    ...
    if(strlen(pszBarcode) > 21)
    {
        ScrPrint(0, 5, ASCII, pszBarcode);
    }
    else 
    {
        ScrPrint((128-6*StrLen(pszBarcode))/2, 5, ASCII, pszBarcode);
    }
}
...
WORD Enter_prod_type(unsigned char *psProdType)
{
	...
	OS_ClearScreen();
 //   ScrPrint(0, 0, CFONT|REVER, "%-16.16s", " SERVICE TYPE  ");
 /*   ScrPrint(0, 2, ASCII, "1-TRAVEL");
    ScrPrint(0, 3, ASCII, "2-MEDICAL");
    ScrPrint(0, 4, ASCII, "3-OTHER");
    ...
    while(1)
    {
        if(0 == kbhit())
        {
            ...
            if(ucKey == KEY1)
            {
                strcpy(psProdType, "TRAVEL");
                break;
            }
            else if(ucKey == KEY2)
            {
                strcpy(psProdType, "MEDICAL");
                break;
            }
            else if(ucKey == KEY3)
            {
                strcpy(psProdType, "OTHER");
                break;
            }
    ...
    return SelectTypeMenu(psProdType);
}

WORD Enter_scan_barcode(unsigned char *psBarcode)
{
	unsigned char szEnterBuff[36+1], szRecvBuf[256];
	unsigned char ucKey, ucRet;
	int	iStrlen, iRecvCnt;
	int iLen=0, iLastKey,iIndex;

	OS_ClearScreen();          
    ScrPrint(0, 0, CFONT|REVER, " INVOICE NUMBER ");
	ScrPrint(0, 2, ASCII, "Alpha numeric value");
    ScrPrint(0, 3, ASCII, "up to 36 characters");
...
*/
	...
	sprintf((char *)psBarcode, "%s", (char *)szEnterBuff);
	return STAT_OK;
}
...
WORD Enter_Item_Price(BYTE *psPriceBcd)
{
    ...
    strcpy(MainMSG.CSTMSG1, "  SERVICE PRICE ");
...
BYTE InputAmountExt(MSG_Struct * stMSG, BYTE * RetBuf, BOOLEAN ZeroInputOK, sint minlen, sint maxlen, char *CurDes)
{
	...
	while (RetChar != 0)
    {
        ...
        DebugComSend("szDispBuff:%s CurDes:%s cnt:%d\n", szDispBuff, CurDes, cnt);
		if (ucLen>16)
		{
