...
WORD ListsMenu(void)
{
	...
	while(lwStatus!=STAT_OK)
    {
        ...
        {
            case 1 :
                PrintTransList(OS_TRUE,OS_TRUE, FILE_TR);   // modified by Allen 20151117
                break;
            case 2 :
...
WORD SelectTypeMenu(unsigned char *psProdType)
{
    BYTE i,lbCounter=3;
    BYTE *lpBuf[20];
    BYTE FirstItem;
    BYTE lbMenuLine;
    BYTE MENU[][22]={" SERVICE TYPE  ", "TRAVEL", "MEDICAL", "OTHER"};
   // char *type[] = {"TRAVEL", "MEDICAL", "OTHER"};

    for(i=0; i<=lbCounter; i++)
        lpBuf[i] = MENU[i];
    ...
    while(1)
    {
        lbMenuLine = (BYTE)KeyBoardMenu(lbCounter, (const BYTE **)lpBuf, &FirstItem, 1, 200, 0);

        if(lbMenuLine>=1 && lbMenuLine<=lbCounter)
        {
            strcpy(psProdType, MENU[lbMenuLine]);
            break;
        }
        
        if(lbMenuLine == 0) break;
     }
