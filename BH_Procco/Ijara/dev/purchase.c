...
0x01,
0x00,0x07, 
    0x08,0x18,0x3f,0x7f,0x3f,0x18,0x08 
};
...
//        {
  ...
  //      }
       ...
       /* for (cnt=0; cnt<ii; cnt++)
        ...
        memcpy(Balance,gPTStruct.product[ii].price_bcd, 6);
        AmountEntry(Balance);
        DebugComSend("EntryAmount:%s ",gPTStruct.EntryAmount);
//richard to test Ijara	
/* 
        AlBarakaMerchantConfirm(1);
        AlBarakaMerchantConfirm(2);
        AlBarakaMerchantConfirm(3);
    ...
    while (1)
    {
        ...
        if(iNowPage == 1)
        {
            if (step==1)
            {
                ...
                ScrPrint(0, 0, ASCII, "AlBaraka would like");
                ScrPrint(0, 1, ASCII, "to buy below service");
                ScrPrint(0, 2, ASCII, "from");
                ScrPrint(0, 3, ASCII, "[%.19s]", szBuffer);
            } 
            else if (step==2)
            {
                ...
                PubTrimHeadChars(szBuffer, ' ');
                PubTrimTailChars(szBuffer, ' ');
                ScrPrint(0, 0, ASCII, "[%.13s] would", szBuffer);
                ScrPrint(0, 1, ASCII, "like to sell below");
                ScrPrint(0, 2, ASCII, "service to AlBaraka");
                ScrPrint(0, 3, ASCII, "and the service will");
                ScrPrint(0, 4, ASCII, "be available from");
                ScrPrint(0, 5, ASCII, "acceptance date");
            } 
            else
            {
                ...
                PubTrimTailChars(szBuffer, ' ');
                ScrPrint(0, 0, ASCII, "AlBaraka would like");
                ScrPrint(0, 1, ASCII, "to sell below service");
                ScrPrint(0, 2, ASCII, "to");
                ScrPrint(0, 3, ASCII, "[%.19s]", szBuffer);
            }
            
            ScrPrint(0, 6, ASCII, "TYPE:%16.16s",    gPTStruct.product[0].proddesc);
           // ScrPrint(0, 7, ASCII, "    1-ACCEPT 2-REJECT");
            ScrDrLogoxy(10, 57, sArrow_Down);
            //ScrPrint(0, 7, ASCII, "                 DOWN");
            
        }
        else 
        {
            ScrPrint(0, 0, ASCII, "INVOICE No:", gPTStruct.product[0].barcode);
            ScrPrint(0, 1, ASCII, "%s", gPTStruct.product[0].barcode);
            memset(szBuffer, 0, sizeof(szBuffer));
            FormatAmt(szBuffer, gPTStruct.product[0].price_bcd, gCurrencySymbol, BCD_IN);
            PubTrimHeadChars(szBuffer, ' ');
            ScrPrint(0, 4, ASCII, "SERVICE PRICE:", szBuffer);
            ScrPrint(0, 5, ASCII, "%21.21s", szBuffer);

            ScrPrint(0, 7, ASCII, "    1-ACCEPT 2-REJECT");
            ScrDrLogoxy(2, 57, sArrow_Up);
        }
        ...
        if(ucKey == KEYDOWN)  iNowPage = 2;
        if(ucKey == KEYUP)    iNowPage = 1;
