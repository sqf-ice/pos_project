...
INT OS_ShowMenu(BYTE ItemCount,BYTE **MenuItems,PBYTE FirstItem,BYTE gOldItem,WORD TimeOut,byte byArabicHeader)
{
	...
	if (byArabicHeader) 
	{
		rc =   ShowMenu((menu *)&m, TimeOut, Opt_LONGMSG|Opt_ARABIC);
	}
	else
	{
		rc =   ShowMenu((menu *)&m, TimeOut, Opt_LONGMSG);
	}
	OS_ClearScreen();
	return rc;
}
...
INT GetApplicationConfig(APPLCONFIGSTRUCT *pApplConfig)
{
	...
	// Modified by Kim_LinHB 2013-9-5 0.05.0011
...
WORD StartPrinter(void)
{
    ...
    {
	    //ScrClrLine(4, 7);
		OS_ClearScreen();      //modified by Allen 20151112
...
int	Check_SecurityMenuPW()
{
	BYTE timebuffer[7];
	BYTE pszOut[12];
	BYTE GenPW[5];  // Modified by Kim_LinHB 2013-3-5 build 0.05.0009 from 4 to 5 cuz using sprintf below
	...
	sprintf(GenPW,"%02d%02X",Month+Day,Hour);
