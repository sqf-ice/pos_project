        ...
        if ( Seek(gTrHandle, 0, SEEK_SET) >= 0 )
        {
            ...
            if (NumBytes == 0 )
            {
                s_IslemTipineGoreToplamlar(lInstalmentCount,lLoyaltyCount,
                                            lPaymentCount,/*lAccountMaintCount,*/
                                            lInstalmentAmount,lLoyaltyAmount,
                                            lPaymentAmount/*,lAccountMaintAmount*/);
            }
...
static WORD s_IslemDetaylari(TransStruct *fpTransInfo)
{
    ...
    {
        memcpy(lpLine+5,PRT_LOSMARTDEBIT,4);
    }
    else if (memcmp(fpTransInfo->TransType,TYPE_71,2)==0)
    {
        memcpy(lpLine+5,PRT_LOSMARTLOYALTY,4);
    }
    else if (memcmp(fpTransInfo->TransType,TYPE_72,2)==0)
    {
        memcpy(lpLine+5,PRT_LOSMARTPAYMENT,4);
    }
    //		else if (memcmp(fpTransInfo->TransType,TYPE_73,2)==0)
    //			memcpy(lpLine+5,PRT_LOSMARTACCOUNT_MAINT,4);
    
    lpLine[9]='-';
    memcpy(lpLine+10,fpTransInfo->TransDate,2);
    lpLine[12]='/';
    memcpy(lpLine+13,fpTransInfo->TransDate+2,2);
    lpLine[15]='/';
    memcpy(lpLine+16,fpTransInfo->TransDate+6,2);
    lpLine[18]='-';
    memcpy(lpLine+19,fpTransInfo->TransTime, 2);
    lpLine[21]=':';
    memcpy(lpLine+22,fpTransInfo->TransTime+2,2);
...
static WORD s_PrintTransHeader( BYTE *fpTicketType )
{
    ...
    if ( PrnHead(szTempBuff)==STAT_OK)
    {
		...
		PrintBuffer_SelectFont(szTempBuff, 24);   //add by richard 2015070
        memset(lpLine, 0, sizeof(lpLine));
