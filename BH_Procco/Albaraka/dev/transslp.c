...
WORD Print_SaleDraft_Slip(void)
{
  ...
  /*  if (gLanguage==ARABIC)
   ...
   PrintBuffer_Attr(sPrnBuffer1, PRN_DOUBLE_WIDTH|PRN_DOUBLE_ENGLISH);
    ...
    if (gLanguage==ARABIC)
    {
        PrintBuffer_Arabic(sPrnBuffer1, PRN_ACT_ARABIC_RIGH, " ÇáãÌãæÚ Çáßáí:");
		PrintBuffer_Eng(sPrnBuffer1, "%s\n\n", (char *)lWrkBuf);
	} 
    else
    {
...
WORD Prepare_Print_SaleOrPaymentSlip(BYTE PrintMerhantCopy)
{
    ...
    if (memcmp(gPTStruct.TransType,TYPE_70,2)==0)
    {
        ...
        if (gLanguage==ARABIC)
        {
        	PrintBuffer_Attr(sPrnBuffer1, PRN_DOUBLE_HEIGHT|PRN_DOUBLE_ARABIC);
            PrintBuffer_Eng(sPrnBuffer1, "\n");
            PrintBuffer_Arabic(sPrnBuffer1, PRN_ACT_ARABIC_CENT, "ÚÞÏ ÈíÚ ÈÇáãÑÇÈÍÉ");
			PrintBuffer_Eng(sPrnBuffer1, "\n");
        }
        else
        {
   ...
   /* if(gLanguage==ARABIC) 
    ...
    if (memcmp(gPTStruct.TransType,TYPE_70,2)==0)
    {
        ...
        if (gLanguage==ARABIC)
        {
            PrintBuffer_Arabic(sPrnBuffer1, PRN_ACT_ARABIC_RIGH, "ÍÑÑ åÐÇ ÇáÚÞÏ Èí ßá ãä:");
            PrintBuffer_Arabic(sPrnBuffer1, PRN_ACT_ARABIC_RIGH, "ÇáÈäß (ÇáÈÇÆÚ):");
            strcpy(lWrkBuf, "Èäß ÇáÑßÉ ÇáÅÓáÇãí¡ Ô. ã. È. (ã)");
            len = StrLen(lWrkBuf);
            lWrkBuf[len-3] = '(';
            lWrkBuf[len-1] = ')';
...
*/
    ...
    if (memcmp(gPTStruct.TransType,TYPE_70,2)==0)
    {
        ...
        if (gLanguage==ARABIC)
        {        
            PrintBuffer_Arabic(sPrnBuffer1, PRN_ACT_ARABIC_RIGH, "ÈÇÚ ÇáÈäß ÇáÓáÚ ÇáãÐßæÑÉ ÈíÇäÇÊåÇ");
        ...
        else
        {
            ...
            PrintBuffer_Eng(sPrnBuffer1, "GOODS DESCRIBED BELOW AND THE\n");
            PrintBuffer_Eng(sPrnBuffer1, "CARDHOLDER AGREES TO BUY THE\n");
			PrintBuffer_Eng(sPrnBuffer1, "GOODS ON THE FOLLOWING TERMS:\n\n");
        }
	//-------------
	PrintBuffer_SelectFont(sPrnBuffer1, 16);
	PrintBuffer_Attr(sPrnBuffer1, PRN_DOUBLE_WIDTH|PRN_DOUBLE_ENGLISH);
	for (ii=0; ii<NUM_PRODUCTS_PERTRAN && gPTStruct.product[ii].numbers; ii++)
	{
        ...
        {
			PrintBuffer_Attr(sPrnBuffer1, PRN_DOUBLE_HEIGHT|PRN_DOUBLE_ARABIC);
            PrintBuffer_Arabic(sPrnBuffer1, PRN_ACT_ARABIC_CENT, "ÈíÇäÇÊ ÇáÓÏÇÏ");
        } 
        else
        {
	...
	lWrkBuf[12] = 0;
       if (gLanguage==ARABIC)
        ...
        if (gLanguage==ARABIC)
        {
            ...
            PrintBuffer_Arabic(sPrnBuffer1, PRN_ACT_ARABIC_RIGH, "ÇÇáÓÇÈÞÉ ãä ÍÇãá ÇáÈØÇÞÉ Úáì");
            PrintBuffer_Arabic(sPrnBuffer1, PRN_ACT_ARABIC_RIGH, "ÇáÔÑæØ æÇáÃÍßÇã ÇáãäÙãÉ æÇáÊí");
...
//    PrintBuffer_Eng(sPrnBuffer1, "\n");
    ...
    if (memcmp(gPTStruct.TransType,TYPE_70,2)!=0)
	{
        if (gLanguage==ARABIC)
        {
            PrintBuffer_Arabic(sPrnBuffer1, PRN_ACT_ARABIC_CENT, "ÔßÑÇ áÇÓÊÎÏÇãßã ÈØÇÞÉ ÊÞÓíØ ãä Èäß ÇáÈÑßÉ ÇáÅÓáÇãí");
        } 
        else
        {
            PrintBuffer_Eng(sPrnBuffer1, "   THANK YOU FOR USING ALBARAKA TAQSEET CARD\n");
    ...
    else
    {
        ...
        if (gLanguage==ARABIC)
        {
            ...
            PrintBuffer_Arabic(sPrnBuffer2, PRN_ACT_ARABIC_CENT, "ÔßÑÇ áÇÓÊÎÏÇãßã ÈØÇÞÉ ÊÞÓíØ ãä Èäß ÇáÈÑßÉ ÇáÅÓáÇãí");

			PrintBuffer_Arabic(sPrnBuffer3, PRN_ACT_ARABIC_CENT, "äÓÎÉ ÇáÊÇÌÑ");
			PrintBuffer_Eng(sPrnBuffer3, "\n");
			PrintBuffer_Arabic(sPrnBuffer3, PRN_ACT_ARABIC_RIGH, "ÔßÑÇ áÇÓÊÎÏÇãßã ÈØÇÞÉ ÊÞÓíØ ãä Èäß ÇáÈÑßÉ ÇáÅÓáÇãí");
        } 
        else
        {
            ...
            PrintBuffer_Eng(sPrnBuffer2, "           <<    CUSTOMER COPY    >>            \n\n");
			...
			PrintBuffer_Eng(sPrnBuffer3, "           <<    MERCHANT COPY    >>            \n\n");
			PrintBuffer_SelectFont(sPrnBuffer3, 16);
			PrintBuffer_Eng(sPrnBuffer3, "   THANK YOU FOR USING ALBARAKA TAQSEET CARD\n");
			PrintBuffer_SelectFont(sPrnBuffer3, 24);
        }    
...
WORD PrintTransSlip(BYTE PrintMerhantCopy)
{
    ...
    if ( lwStatus == STAT_OK )
    {
        if(PrintMerhantCopy==OS_TRUE)
        {
            ...
            {
                if (KeyBoardValid("",(PCHAR)DSP_SECOND_COPY_PRINT,"",(PCHAR)DSP_K_NO,"",(PCHAR)DSP_K_YES,
                                KEYB_CANCEL_MASK|KEYB_VAL_MASK, 30)== KEYCANCEL)
				{
					goto THIRD_PRINT;
				}
