...
WORD ParameterDownload(BYTE fbMerchantPhones, BYTE fAutoParam, BYTE Connect)
{
    ...
    OS_ClearScreen(); 
    lwStatus = StartParameterDload(fAutoParam);     //这里返回的失败
    Comms_Disconnect();
...
WORD StartParameterDload(BYTE fAutoParam )
{
    ...
    ///Parameter Request///////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////////
    DisplayLongString(DSP_PARAM_REQUEST,0);    //显示SEND REQUEST	
	//by jarod  download para
	lwStatus = ParameterRequest(lpWrkBuf, fAutoParam);       //这个函数没有返回OK
    if (lwStatus != STAT_OK)      
    {
        return(lwStatus);
    }
    ...
    ///Version Control////////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////////
    DisplayString(DSP_PARAM_DLOADING,0);    //但是没有显示这句
...
static WORD ParameterRequest(BYTE *fpWrkBuf,BYTE fAutoParam)
{
	...
	//ScrPrint(0, 2, ASCII, "Status = %d", lwStatus);
	//getkey();
	//OS_ClearScreen();
    if (lwStatus!=STAT_OK)
        return(STAT_NOK);
...
static WORD GeneralParameter(BYTE *fpWrkBuf)
{
    ...
    AscToHex((PCHAR)fpWrkBuf+lbLen,(PCHAR)lpTime,2);
	//AscToHex(CurrCode,(PCHAR)lpTime,2); 
...
static WORD SchemeList(BYTE *fpWrkBuf, WORD *fpBlockNo)
{
    ...
    PrintBuffer_SelectFont(sPrnBuffer1, 24); //add by richard 20150701
