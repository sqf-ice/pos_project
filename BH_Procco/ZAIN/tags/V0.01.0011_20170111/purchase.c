...
{    
    ...
    */
	...
	//ComOutput(gPTStruct.TransType,2,"purchase.c_gPTStruct.TransTypeline:%d",__LINE__);
    if (memcmp(gPTStruct.TransType,TYPE_73,2)!=0)
    {
	    ...
	    iRet = AddRefNum(gPTStruct.RetrivalRef);//added by jeff_xiehuan
		//ComOutput(&gPTStruct,sizeof(PTStruct),"firstgPd\n");
		//ComOutput(gPTStruct.RetrivalRef,12,"Add Trans,gPTStruct.RetrivalRef,iRet=%d",iRet);
