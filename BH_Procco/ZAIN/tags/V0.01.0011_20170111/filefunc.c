...
WORD AddRefNum(const char *RefNum)
{
	int iFd;
...
int CheckRetrivalRef(unsigned char* RetrivalRef) //added by jeff_xiehuan
{
    ...
    if (iFd >= 0)
    {
		...
		while(1) 
		{
			...
			//ComOutput(ucRead,12,"%dcompare,ucRead",2);
			if(memcmp(ucRead, RetrivalRef, 12) == 0)
			{
				//ComOutput(ucRead,12,"fileOutputRef");
				Close(iFd);
				return STAT_OK;
			}
		}
