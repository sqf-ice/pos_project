...
BYTE StartBatchClose(void)
{
    ...
    else
    {
		...
		FileInit(FILE_REF);            //added by jeff_xiehuan20161216
...
static WORD SendBatchHeader_Details(void)  
{
	...
	//BYTE lCounterReversal=0;    //added by Allen 2016-09-22
    BYTE Len=0;
	...
	//memset(gAmountReversal, ' ', LG_AMOUNT+1);    //added by Allen 2016-09-22
		...
		if ( Seek(gTrHandle, 0, SEEK_SET) >= 0 )
        {
            do
            {
                ...
                if (NumBytes>0)
                {
					...
					/*
					//added by Allen 2016-09-22
					if(memcmp(gPTStruct.TransType,TYPE_72,2)==0) //REVERSAL
					{
						AddAscii(gPTStruct.EntryAmount, LG_AMOUNT, gAmountReversal, LG_AMOUNT, gAmountReversal, LG_AMOUNT);
						lCounterReversal++;
					}
					//add end
					*/
...
/*
	//added by Allen 2016-09-22
    gAmountReversal[12]=0;
    StrTrim(gAmountReversal,gAmountReversal,' ');
    StrLeftPad(gAmountReversal,12,gAmountReversal,'0');
    lCounterReversal++;//For Rec.Adv.
    itoa(lCounterReversal,    (PCHAR)lWrkBuf, 10);
    lWrkBuf[3]=0;
    StrTrim(lWrkBuf,lWrkBuf,' ');
    StrLeftPad(lWrkBuf,3+1,lCounterReversal,'0');
	//add end
  */  
...
if(gDebug)
{
                ...
                do
                {
                    ...
                    if ( NumBytes>0)
                    {
                        ...
                        if (sw==STAT_OK)//Response Code OK -- PARTIAL
                        {
                            if(memcmp(gPTStruct.TransType,TYPE_70,2)==0)//INSTALLMENT
                            {
                             //   PrepareField55(OS_TRUE,OS_TRUE);  removed by Allen 2016-09-06
        ...
        else   // change the amount format from 000000000000 to 000000000.000, then send it to host
        {
...
static WORD PrepareBatchSlip(BYTE Success/*BYTE *fpWrkBuf, BYTE *fpRecordCounter,BYTE *fpTotalAmount*/)
{
    ...
    if(!Success)       //modify by richard 20150701
    {
