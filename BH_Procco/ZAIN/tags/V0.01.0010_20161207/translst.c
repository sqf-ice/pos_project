...
static WORD s_IslemDetaylari(TransStruct *fpTransInfo)
{
    ...
    if (memcmp(fpTransInfo->TransType,TYPE_72,2)==0)
    {
    ...
    else if (memcmp(fpTransInfo->TransType,TYPE_70,2)==0)
    {
	...
	{
		memcpy(lpLine+5,PRT_LOSMARTREVERSAL,8);
	}
	//add end
	...
	{
		lpLine[13]='-';
		memcpy(lpLine+14,fpTransInfo->TransDate,2);
		lpLine[16]='/';
		memcpy(lpLine+17,fpTransInfo->TransDate+2,2);
		lpLine[19]='/';
		memcpy(lpLine+20,fpTransInfo->TransDate+6,2);
		lpLine[22]='-';
		memcpy(lpLine+23,fpTransInfo->TransTime, 2);
		lpLine[25]=':';
		memcpy(lpLine+26,fpTransInfo->TransTime+2,2);
	...
	else
	{
		...
		lpLine[20]='-';
		memcpy(lpLine+21,fpTransInfo->TransTime, 2);
		lpLine[23]=':';
