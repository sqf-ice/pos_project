...
int cEMVGetHolderPwd(int iTryFlag, int iRemainCnt, uchar *pszPlainPin)
{
	...
	PubDispString("PLS ENTER PIN", 4|DISP_LINE_LEFT);

	if (pszPlainPin==NULL && usePinPad == 0)
	{
		return EMV_OK;
	}
	...
	if (1)  // S80
	{
		...
		if (usePinPad == 1){                   // added by Allen 20151127
			PubDispString("PLS USE PINPAD", 6|DISP_LINE_LEFT);
			ucRet = PPScrCls();
			PPScrPrint(0, 0, "PLS ENTER PIN");
			PPScrClrLine(1);
			if( ucRet )
			{
				// EMV core has processed PIN entry and PED failed.
			...
			ucRet = PPEmvGetPwd(4, 8, sPinBlock);	
			switch( ucRet )
			{
			case 0x00:
...
int usEMVGetTLVFromColxn(WORD usTag, BYTE *psData, WORD *pLen)
{
    ...
    iRet = EMVGetTLVData(usTag, psData, &iOutLen);
    if (iRet==EMV_OK)
    {
        *pLen = (WORD)iOutLen;
    }
