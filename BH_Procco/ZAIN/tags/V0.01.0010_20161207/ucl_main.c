...
#define LEN_WORKBUF			(1024*4)      //added by Allen 2016-05-20
#define STX             0x02     //added by Allen 2016-05-20
...
static uchar		sg_sWorkBuf[LEN_WORKBUF+50];  //added by Allen 2016-05-20
...
int Try_Gprs_NectConnect(char *ip_str, short port, int *pSock, int TimeOutSec)
{
	...
	for (ii=0; ii<3; ii++)
	{
		*pSock = NetSocket(NET_AF_INET, NET_SOCK_STREAM, 0);        //创建socket
		if (*pSock < 0)
        {
			//return -1;
			if (2 == ii)    // modified by Allen 20151110
			{
				return -1; 
			}
			else
        ...
        ScrPrint(0,7,0,"%s:%d",ip_str, (int)(ushort)port);           //这里都是不会变的
...
sint UCL_Tcpip_Connect(TcpipParamStruct* prm, ushort uiTimeOutSec)
{
	...
	// Setup socket
	iRet = NetSocket(NET_AF_INET, NET_SOCK_STREAM, 0);      //创建socket
	if (iRet < 0)
	{
		return iRet;	
	}
	...
	//ScrPrint(0, 2, ASCII, "socket: %d", iRet);
	//getkey();
	//OS_ClearScreen();		
	sg_iSocket = iRet;
	...
	iRet = Netioctl(sg_iSocket, CMD_IO_SET, 0);
	if (iRet!=0)
	{
		return iRet;
	}
...
int UCL_Tcpip_Send(TcpipParamStruct* prm, byte *buffer, usint buffer_size,ushort uiTimeOutSec)
{
	...
	if (iRet < 0)
	{
		...
		//ScrPrint(0, 0, ASCII, "Allen test");
		//ScrPrint(0, 2, ASCII, "Netioctl 1: %d", iRet);
		//getkey();
		//OS_ClearScreen();
		return iRet;
	}
	...
	while(1)
	{
		...
		if (iRet < 0)
		{
			//OS_ClearScreen();                                        //add by Allen 20151110
			//ScrPrint(0, 0, ASCII, "Allen test");
			//ScrPrint(0, 2, ASCII, "Netioctl 2: %d", iRet);
			//getkey();
			//OS_ClearScreen();
			return iRet;
		}
...
sint UCL_Tcpip_Receive(TcpipParamStruct* prm, byte *buffer, usint *buffer_size, ushort uiTimeOutSec)
{
	...
	DelayMs(200);    // add by Allen 20151110
	iRet = 	Netioctl(sg_iSocket, CMD_TO_SET, uiTimeOutSec*1000);
	if (iRet < 0)
	{
		//OS_ClearScreen();                                        //add by Allen 20151110
		//ScrPrint(0, 0, ASCII, "Allen test");
		//ScrPrint(0, 2, ASCII, "RNetioctl: %d", iRet);
		//getkey();
		//OS_ClearScreen();
        return iRet;
	}
...
int PSTNDial(uchar ucDialMode, COMM_PARA *CommParam, char *pTelNo, char pMode)
{
	...
	{
		OnHook();
		ucRet = ModemDial(CommParam, pTelNo, 0);
		if( ucRet!=0 )
		{
			//DebugComSend("ModemDial1 = %d\n", ucRet);
	...
	// redial here.
	OnHook();
	ucRet = ModemDial(CommParam, pTelNo, 1);
	//DebugComSend("ModemDial2 = %d\n", ucRet);
	if( ucRet==0x00 )
	{
		if( pMode ==CM_ASYNC )
		{
