...
WORD TransMenu(BYTE fbBalance,BYTE fbVoidReversal,BYTE fbInstallment,BYTE fbLoyalty,BYTE fbPayment,BYTE fbAccountMaint)
{
    BYTE lbMenuLine;
    BYTE FirstItem;
    ...
    BYTE lbLoyalty;
    BYTE lbPayment;
	BYTE lbVoidReversal;   //added by Allen for ZAIN 2016-05-03
    BYTE lbAccountMaint;
	...
	lbPayment     =99;
	lbAccountMaint=99;
	lbVoidReversal = 99;   //added by Allen for ZAIN 2016-05-03
	...
	if (fbInstallment)
	{
		...
		lpBuf[lbCounter] = DSP_POINTS_REWARD_SALE;   //modified by Allen for ZAIN 2016-05-03
		lbBufCounter+=12;
	}
	/*if (fbLoyalty)
	{
	...
	if (fbPayment)
	{
		...
		lpBuf[lbCounter] = DSP_REDEEM; //modified by Allen for ZAIN 2016-05-03
		lbBufCounter+=12;
	}
	...
	{
		lbBalance = ++lbCounter;
		//lpBuf[lbCounter] = DSP_CHECK_BALANCE;
		lpBuf[lbCounter] = DSP_CHECK_BALANCE;//add by lqc 20150619
		lbBufCounter+=12;
	}
	//added by Allen for ZAIN 2016-05-03
	//if (fbVoidReversal)
	//{
	//	lbVoidReversal = ++lbCounter;
	//	lpBuf[lbCounter] = DSP_VOID_REVERSAL;
	//	lbBufCounter+=12;
	//}
	//if (fbVoidReversal)    //modified by jeff_xiehuan20161214
	{
		lbVoidReversal = ++lbCounter;
		lpBuf[lbCounter] = DSP_VOID_REVERSAL;
		lbBufCounter+=12;
...
//	lpBuf[lbCounter] = DSP_END_OF_SHIFT;  //remove by richard 20150630
//	lbBufCounter += 12;
	...
	{	
		//ScrCls();
		//Lcdprintf("TranType = BALANCE_LIST");//leeDebug
		//getkey();
	...
	{
		//ScrCls();
		//Lcdprintf("TranType = LAST10TRANSACTION");//leeDebug
		//getkey();
	...
	{
		//ScrCls();
		//Lcdprintf("TranType = INSTALLMENT_PLAN");//leeDebug
		//getkey();
	...
	{
		//ScrCls();
		//Lcdprintf("TranType = LOYALTY");//leeDebug
		//getkey();
		lwStatus = LOYALTY;
	}
	...
	{
		//ScrCls();
		//Lcdprintf("TranType = PAYMENT");//leeDebug
		//getkey();
		lwStatus = PAYMENT;
	}
	...
	{
		//ScrCls();
		//Lcdprintf("TranType = ACCOUNT_MAINT");//leeDebug
		//getkey();
	...
	{
		//ScrCls();
		//Lcdprintf("TranType = TENURE_CALCULATION");//leeDebug
		//getkey();
		lwStatus = TENURE_CALCULATION;
	}
	//added by Allen for ZAIN 2016-05-03
	else if(lbVoidReversal == lbMenuLine)  
	{
		//if(fbVoidReversal == 0)    //added by jeff_xiehuan20161214
		//{
		//	ScrCls();
		...
		//}
		lwStatus = VOID_REVERSAL;
	}
	//add end
	else
	{
		//ScrCls();
		//Lcdprintf("TranType = CANCEL");//leeDebug
		//getkey();
		lwStatus = CANCEL;
	}
	return(lwStatus);
...
WORD ListsMenu(void)
{
	...
	while(lwStatus!=STAT_OK)
    {
        ...
        {
            case 1 :
                PrintTransList(OS_TRUE,OS_TRUE, FILE_TR);   // modified by Allen 20151116
                break;
            case 2 :
				PrintTransList(OS_FALSE,OS_TRUE, FILE_TR);   // modified by Allen 20151116
				break;
			case 3 :
				ParameterList();
				break;
...
WORD ConnectMode(void)
{
	...
	lbMenuLine=KeyBoardMenu(2,gConnectMenu,&FirstItem,OldItem,200,0);
...
WORD ReversalMode(void)
{
	...
	lbMenuLine=KeyBoardMenu(2,gReversalMenu,&FirstItem,OldItem,200,0);
	
	switch (lbMenuLine)
	{
		case 1 :
			gReversalType = ISO_VOIDREVERSAL_REWARD;
			break;
		case 2 :
			gReversalType = ISO_VOIDREVERSAL_REDEEM;
			break;
		default :
			return(STAT_NOK);
	}
...
//New Calculation
		...
		//Tek Tip Bilet varsa menu g攔乶t乴enmeden gec
		else
		{
...
WORD TransactionTypeMenu(void)
{
	...
	lbBalance       = 1;
	//added by Allen 2016-11-30
	GetEODFlag();
	if(gEODParam.EODFlag == 1)
	{
		lbVoidReversal = 1;
	}
	else
	...
	//add end
	//lbVoidReversal = 1;  //added by Allen for ZAIN 2016-05-03  //removed by Allen 2016-11-30
	lbLastTrans     = 0;
    //!!!sel??? sonra at	
...
/*	if((lbInstallment==0)&&(lbLoyalty==0)&&(lbPayment==0)&&(lbAccountMaint==0))
    ...
    if((lbInstallment==1)&&(lbLoyalty==0)&&(lbPayment==0)&&(lbAccountMaint==0))   //确定交易的类型！！
	{
	...
	lbRet = TransMenu(lbBalance,lbVoidReversal,lbInstallment,lbLoyalty,lbPayment,lbAccountMaint);
	switch(lbRet)                                                   //确定这次交易的类型！！！！！
	{
		case INSTALLMENT_PLAN:      
			//memcpy(gPTStruct.TransType,TYPE_70,2);
			memcpy(gPTStruct.TransType,TYPE_72,2);   //modified by Allen for ZAIN 2016-05-16
			return(STAT_OK);
		case LOYALTY:
			memcpy(gPTStruct.TransType,TYPE_71,2);
			return(STAT_OK);
		case PAYMENT:
			//memcpy(gPTStruct.TransType,TYPE_72,2);
			memcpy(gPTStruct.TransType,TYPE_70,2);    //modified by Allen for ZAIN 2016-05-16
			//bBalanceFromOptions = FALSE; //// Added by Kim_LinHB 2013-3-8 build 0.05.0009 
			bBalanceFromOptions = 1;  //added by jeff_xiehuan 20161206
			return(STAT_OK);
		case ACCOUNT_MAINT:
			memcpy(gPTStruct.TransType,TYPE_73,2);
			return(STAT_OK);
...
//			Last10Transaction();
		...
		//added by Allen for ZAIN 2016-05-03
		case VOID_REVERSAL:
			memcpy(gPTStruct.TransType,TYPE_76,2);
			return (STAT_OK);
		//add end
		case CANCEL:
			return(STAT_NOK);
...
// 			RateValue = 60;
		...
		if(RateValue>0)
		{
			...
			if(memcmp(Buffer,gPTStruct.Balance,12)<=0)
			{
//by jarod  2011.06.09
...
/*   modify by richard 20150701  not need tenure screen
TENURE_MENU:
    memset(&MENU,0,sizeof(MENU));
	lbCounter=0;
