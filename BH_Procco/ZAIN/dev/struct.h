...
#define NUM_PRODUCTS_PERTRAN    5
	...
	//added by Allen //added by Allen for ZAIN  2016-05-16
	BYTE InvoiceNum[LG_INVOICENUM];  
	BYTE PIN[LG_PIN]; 
	BYTE PointsBalance[LG_AMOUNT];
	BYTE EarnedPoints[LG_AMOUNT];
	BYTE ReversalPoints[LG_AMOUNT];
	//add end 
} PTStruct;
...
typedef struct
{
	...
	char		szTelNo[20+1];	
	COMM_PARA	stPara;
    char       ucSignalLevel;  // dial voltage level, range 1-15. Use 0 for disable.
}Dial_upStruct;
#define LG_DIAL_UPSTRUCT   (sizeof(Dial_upStruct))

typedef struct
{
	char EODFlag;
	unsigned char szDateTime[7];
}EODPARAM;

typedef struct saveRetrivalRef
{
	char retrivalRef[12];
	int tranType;
	int hasVoidFlag;
...
#define LG_EODPARAM (sizeof(EODPARAM))
extern EODPARAM gEODParam;
//add end
#endif
