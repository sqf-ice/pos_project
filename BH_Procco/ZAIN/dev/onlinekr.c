...
extern int gl_iSendTagC5;    //added by jeff_xiehuan20161206
extern BYTE gl_ucCashAmount[12];
...
void ISO_DisPlayErrMsg(byte*Field_Attr)
{
	...
	ComOutput((uchar*)"",0,"FileName:OnlineKr.c,Line:%d,ISOData.Field39 = %c%c,Recp_Code = %d",__LINE__,ISOData.Field39[0],ISOData.Field39[1],Recp_Code);
	for(i=0;i<12;i++)
	{
		if(gHostErrMsgTableFlag[i]==Recp_Code)
		{
			if((Recp_Code == 9) && (memcmp(gPTStruct.TransType,TYPE_76,2)!=0))  //added by Allen 2016-09-28
				return;
			//added by Allen 2016-11-30
			if(ISOData.Field39[0] == '1' && ISOData.Field39[1] == '4')
			{
				Print_HostErr_Slip((BYTE *)"CARD CLOSED");
				DisplayLongString((BYTE *)"CARD CLOSED",2000*100);
				ScrCls();		
				return;
			}
			else if(ISOData.Field39[0] == '5' && ISOData.Field39[1] == '7')
			{
				Print_HostErr_Slip((BYTE *)"CARD BLOCK");
				DisplayLongString((BYTE *)"CARD BLOCK",2000*100);
				ScrCls();		
				return;			
			}
			//add end
...
WORD ISO_SendReceive(BYTE Connect,BYTE Type)        //根据交易类型不同，组成不同的数据包，并且发送/接收
{
	...
	ComOutput("\x02\x02",2,"fileName:onlinekr.c,line:%d now the tran type is %c%c",__LINE__,gPTStruct.TransType[0],gPTStruct.TransType[1]);
...
#if 1  //added by jeff_xiehuan 20161206
	memcpy(ISOData.Field14,"\x49\x12",2);
#endif
  /* if(1  == 1)
   {
	   int i = 0;
	   ...
	   memset(printTest,0,sizeof(printTest));
	   for(i = 0;i < 2;i++)
	   {
		   sprintf(printTest + i*3,"%02X ",*(ISOData.Field14 + i));
	   }
	   ScrCls();
	   ScrPrint(0,0,0,"test print");
	   ScrPrint(0,2,0,"%s",printTest);
	   getkey();
   }*/
    ///////////////////////////////////////////////////
    ///Country Code
...
#if 1
	if(1 == 1)  //added by jeff_xiehuan20161207
	{
		int i; 
	
		for(i = 0;i < strlen(ISOData.Field35);i++)
		{
			if(ISOData.Field35[i] == '=')
			{
				memcpy(&ISOData.Field35[i + 1],"4912",4);
				break;
			}
		}
...
#endif
//#if 0
//	memcpy(ISOData.Field35 + 18,"4912",4);
//#endif 
//if(1 == 1)
//{
//	 
...
//	   for(i = 0;i < 30;i++)
//	   {
//		   sprintf(printTest + i*3,"%02X ",*(ISOData.Field35 + i));
//	   }
//	   ScrCls();
//	   ScrPrint(0,0,0,"test print");
//	   ScrPrint(0,1,0,"%s",printTest);
//	   getkey();
//
//}
	...
	else if(Type==ISO_INSTALLMENT_ADVICES || Type == ISO_RETAIL)
	{
		...
		memcpy(ISOData.Field47+lbCount,"\xC1\x04\x54", 3);               lbCount=lbCount+3;
		memcpy(ISOData.Field47+lbCount,"POS", 3);                        lbCount=lbCount+3;
		if(gl_iSendTagC5 == 1)  //added by jeff_xiehuan 20161206
		{
			/*ScrCls();
			ScrPrint(0,3,0,"pass TagC5");
			getkey();*/
			//gl_iSendTagC5 = 0;
			memcpy(ISOData.Field47+lbCount,"\xC5\x07\x46", 3);            lbCount=lbCount+3;
			memcpy(ISOData.Field47+lbCount,gl_ucCashAmount, 6);           lbCount=lbCount+6;
		}
        //---------------------------------------------------------------------------------
...
//	for(i = 0;i < length;i++)
//	{
//		sprintf(printBuf + i*3,"%02X ",TempBuf[i]);
//	}
...
//Earn points = conversion rate * amount
int CaculatePoints(unsigned char *ConversionRate, unsigned char *EarnedPoints)
{
	int iConversionRate, i, iLength = 0;
	double dAmount, dEarnedPoints;
	unsigned char *pAmountBegin, *pErnPoins, *pTmp;
	unsigned char ucAmount[13] = {0};
	unsigned char ucTmp[13] = {0};
	...
	sprintf(ucTmp, "%.4lf", dEarnedPoints);
	//added by Allen 2016-11-23
	pTmp = ucTmp;
	iLength = strlen(ucTmp);
	while(*pTmp != 0)
		pTmp++;
	pTmp--;
	if(*pTmp >= '5')
	{
		...
		{
			*pTmp = 0;
			sprintf(EarnedPoints, "%c%s", '1', ucTmp);
		}
		else
		{
			*pTmp = 0;
			memcpy(EarnedPoints, ucTmp, strlen(ucTmp));
		}
	}
	else
	{
...
int Parse47_Points(unsigned char *err_code, unsigned char Type)
{
	...
	if ( Tag = FindStr(F0, lenF0, "\xD3", 1))
	{
		...
		//added by jeff_xiehuan 20161206
		else if( ( (Type == ISO_GETBALANCE) && (memcmp(gPTStruct.TransType,TYPE_72,2)==0) )); //||  ( (Type == ISO_GETBALANCE) && (memcmp(gPTStruct.TransType,TYPE_70,2)==0) ))  // for balance inquiry D3 means conversion rate
		{
			sprintf(ConversionRate, "%*.*s",lenTag*2, lenTag*2, Value);
			if(ConversionRate[lenTag*2-1] == 'F')
			{
