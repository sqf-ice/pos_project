...
static unsigned char Prn_SaleCodes_FourE_Voucher(SAV_PARAM stSav, TRANS_DATA stData, unsigned char Mode,int saleIndex)
{
	...
	int voucherLen = 0;
	int iTmpLen = 0;
	uchar voucherBuff[100] = {0};
	int v = 0;
	int hasBlankFlag = 0;
	int hasGrepFlag = 0;
	int voucherCount= 0;
#ifdef JEFF_TEST
		DebugComSend("File_%s,LineNo:%d,func=%s,original voucher=%s,len=%d",__FILE__,__LINE__,__FUNCTION__,stData.stVoucher.szNum
	...
	do{
		...
		DebugComSend("File_%s,LineNo:%d,func=%s,Before voucher=%s,len=%d",__FILE__,__LINE__,__FUNCTION__,stData.stVoucher.szNum
			,strlen(stData.stVoucher.szNum));
#endif
		//strcpy(stData.stVoucher.szNum,"38W8NF6X3WHG83Q8XVBJ6M120");//for test
		for(v = 0;v < strlen(stData.stVoucher.szNum);v++)
		{
			if(stData.stVoucher.szNum[v] == '-')
			{
				hasGrepFlag = 1;
			}
			else if(stData.stVoucher.szNum[v] == ' ')
			{
				hasBlankFlag = 1;
			}
		}
			for(v = 0;v < strlen(stData.stVoucher.szNum);v++)
			{
				if(hasGrepFlag)
				{
					voucherBuff[voucherCount] = stData.stVoucher.szNum[v];
					voucherCount++;
				}

				else if(hasBlankFlag)
				{
					voucherBuff[voucherCount] = stData.stVoucher.szNum[v];
					if(stData.stVoucher.szNum[v] == ' ')
					{
						voucherBuff[voucherCount] = '-';
					}
					voucherCount++;
				}
				else if(!hasGrepFlag && !hasBlankFlag)
				{
					voucherBuff[voucherCount] = stData.stVoucher.szNum[v];
					voucherCount++;
					if(((v + 1) % 4) == 0)
			...
			if(voucherBuff[voucherCount-1] == '-')
			{
				voucherBuff[voucherCount-1] = '\0';
			}
		   voucherLen = strlen(voucherBuff);
		  // DebugComSend("now voucherLen=%d,voucherCount=%d,voucherBuff=%s",voucherLen,voucherCount,voucherBuff);
		   if(voucherLen <= 10)
		   {
				vReversalStr(voucherBuff, szBufferPrint); // Added by Kim_LinHB 2013-3-20 v4.56 v4.56
				PrintBuffer_AR(szBufferPrint, PRN_MIDD, SIZE_LARGE_BOLD,0);
		   }
		   else
		   {
			   while(1)
			   {
				   if(voucherLen >= 10)
				   {
					   ...
					   memcpy(ucTmp,voucherBuff + iTmpLen,10);
					   vReversalStr(ucTmp, szBufferPrint);
					   PrintBuffer_AR(szBufferPrint, PRN_MIDD, SIZE_LARGE_BOLD,0);
					   voucherLen -= 10;
					   iTmpLen += 10;
				   }
				   else if(voucherLen > 0 && voucherLen < 10)
				   {
					    memset(ucTmp,0,sizeof(ucTmp));
						memcpy(ucTmp,voucherBuff + iTmpLen,voucherLen);
					    if(voucherLen == 1)
						{
							
							 PrintBuffer_AR(ucTmp, PRN_MIDD, SIZE_LARGE_BOLD,0);
						}
						else
						{
							vReversalStr(ucTmp, szBufferPrint);
