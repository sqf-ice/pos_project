...
static unsigned char Prn_SaleCodes_FourE_Voucher(SAV_PARAM stSav, TRANS_DATA stData, unsigned char Mode,int saleIndex)
{
	...
	unsigned char bPrintedActivation = FALSE;
	//vReversalStr("2«·–Â«» ≈·Ï",szBufferPrint);
	//PrintBuffer(szBufferPrint,PRN_LEFT,SIZE_NORMAL,0);//tets
	//PrintBuffer("Playstation_Store",PRN_RIGHT,SIZE_NORMAL,0);//tets
	//Jump_Lines(1);

	//PrintBuffer("2«·–Â«» ≈·Ï",PRN_LEFT,SIZE_NORMAL,0);//tets
	//PrintBuffer("Playstation_Store",PRN_RIGHT,SIZE_NORMAL,0);//tets
	//Jump_Lines(1);
	//PrintBuffer("2Playstation_Store«·–Â«» ≈·Ï -2",PRN_LEFT,SIZE_NORMAL,0);//tets
	do{
		sg_bNoPaperCountinue = FALSE;
		sg_bCanCelPrn = FALSE;
		...
		case PSN_SUB:
			...
			Jump_Lines(1);
#ifdef FARSI
			PrintBuffer("ÿ—Ìﬁ… «·≈” Œœ«„: ",PRN_LEFT,SIZE_NORMAL,-3);
			Jump_Lines(1);
			PrintBuffer("1-  ”ÃÌ· «·œŒÊ· ·Õ”«»",PRN_LEFT,SIZE_NORMAL,-3);
			PrintBuffer("PS4, PS3, PSP, PC",PRN_RIGHT,SIZE_NORMAL,-3);
			//PrintBuffer("Playstation_Store«·–Â«» ≈·Ï -2",PRN_LEFT,SIZE_NORMAL,0);
			//PrintBuffer("Playstation_Store2-«·–Â«» ≈·Ï ",PRN_LEFT,SIZE_NORMAL,0);
			PrintBuffer("2«·–Â«» ≈·Ï",PRN_LEFT,SIZE_NORMAL,0);
			PrintBuffer("Playstation_Store",PRN_RIGHT,SIZE_NORMAL,0);
			PrintBuffer("RedeemÊ≈Œ Ì«—",PRN_LEFT,SIZE_NORMAL,-3);
			PrintBuffer("3-√œŒ· —ﬁ„ «·ﬁ”Ì„….",PRN_LEFT,SIZE_NORMAL,-3);
			Jump_Lines(1);
#endif 
			break;
		case XBOX_GIFT:
			...
			Jump_Lines(1);
#ifdef FARSI
			PrintBuffer("ÿ—Ìﬁ… «·≈” Œœ«„:",PRN_LEFT,SIZE_NORMAL,0);
			Jump_Lines(1);
			PrintBuffer("1-«·÷€ÿ ⁄·Ï “— «·≈—‘«œ ›Ì",PRN_LEFT,SIZE_NORMAL,-3);
			PrintBuffer("xbox 360 √Ê xbox one",PRN_LEFT,SIZE_NORMAL,-3);
			PrintBuffer("Ê«·–Â«» ≈·Ï Œ«‰… «·”Êﬁ.",PRN_LEFT,SIZE_NORMAL,-3);
			PrintBuffer("2-√Œ —  ›⁄Ì· «·ﬂÊœ ÊÀ„ ﬁ„",PRN_LEFT,SIZE_NORMAL,-3);
			PrintBuffer("≈œŒ«· —ﬁ„ «·ﬁ”Ì„….",PRN_LEFT,SIZE_NORMAL,-3);
			Jump_Lines(1);
#endif 
			break;
		case GOOGLE_PlAY:
			...
			PrintBuffer("ÿ—Ìﬁ… «·≈” Œœ«„:",PRN_LEFT,SIZE_NORMAL,-3);
			Jump_Lines(1);
			PrintBuffer("1-«·–Â«» ≈·Ï",PRN_LEFT,SIZE_NORMAL,-3);
			PrintBuffer("play.google.com",PRN_RIGHT,SIZE_NORMAL,-3);
			PrintBuffer("RedeemÊ«Œ Ì«— ",PRN_LEFT,SIZE_NORMAL,-3);
			PrintBuffer("2-√œŒ· «·—ﬁ„ «·„ÊÃÊœ ⁄·Ï «·ﬁ”Ì„…",PRN_LEFT,SIZE_NORMAL,-3);
			PrintBuffer("›Ì «·Œ«‰… «·„Õœœ….",PRN_LEFT,SIZE_NORMAL,-3);
			Jump_Lines(1);
#endif
		...
		case ITUNES_GIFT:
			...
			PrintBuffer("ÿ—Ìﬁ… «·≈” Œœ«„: ",PRN_LEFT,SIZE_NORMAL,-3);
			Jump_Lines(1);
			PrintBuffer("iTines Store. 1-√Œ Ì«—  ÿ»Ìﬁ",PRN_LEFT,SIZE_NORMAL,-3);
			PrintBuffer("2-«·÷€ÿ ⁄·Ï",PRN_LEFT,SIZE_NORMAL,-3);
			PrintBuffer("Redeem",PRN_RIGHT,SIZE_NORMAL,-3);
			PrintBuffer("Ê«œŒ«· —ﬁ„ ",PRN_LEFT,SIZE_NORMAL,-3);
			PrintBuffer("«·ﬁ”Ì„… «·„ﬂÊ‰ „‰ 16 Œ«‰….",PRN_LEFT,SIZE_NORMAL,-3);
			Jump_Lines(1);
#endif 
			break;
		default:
...
static unsigned char Prn_SaleCodes_JAWWY(SAV_PARAM stSav, TRANS_DATA stData, unsigned char Mode)
{
	...
	do{
		...
		if(PRN_CUSTOMER == Mode)
		{
			...
			Jump_Lines(1);
#ifdef FARSI  
			PrintBuffer("·‘Õ‰ —’Ìœﬂ° «› Õ  ÿ»Ìﬁ ÃÊ¯Ì", PRN_LEFT, SIZE_NORMAL,-3);
			PrintBuffer("Ê«÷€ÿ ⁄·Ï\"Õ”«»Ì\"°  Ê«ﬂ » «·—ﬁ„ ", PRN_LEFT, SIZE_NORMAL,-3);
			//PrintBuffer("«·„ﬂÊ‰ „‰ 14 Œ«‰… «·„ﬂ Ê»⁄·Ï", PRN_LEFT, SIZE_NORMAL,-3);
			PrintBuffer("«·„ﬂÊ‰ „‰ 14 Œ«‰… «·„ﬂ Ê»", PRN_LEFT, SIZE_NORMAL,-3);
			PrintBuffer("⁄·Ï Â–« «·≈Ì’«·.", PRN_LEFT, SIZE_NORMAL,-3);
			Jump_Lines(1);
#endif
...
// 			Prn_A_LineOf("*");
...
#ifdef FARSI
			sprintf(szBufferPrint, "%s%34s", "Product name:", "‰Ê⁄ «·„‰ Ã");
#else
		...
		if(PRN_CUSTOMER == Mode)
		{
...
#ifdef FARSI
			sprintf(szBufferPrint, "%s%26s","Activation Number", "—ﬁ„ «· ›⁄Ì·");
			PrintBuffer(szBufferPrint, PRN_MIDD, SIZE_DEMI,0);
			...
			sprintf(szBufferPrint, "%s%30s", "INSTRUCTIONS", "«· ⁄·Ì„« ");
			PrintBuffer(szBufferPrint, PRN_MIDD, SIZE_DEMI,-5);

			PrintBuffer("·‘Õ‰ «·»ÿ«ﬁ…", PRN_LEFT, SIZE_NORMAL_BOLD,0);
			PrintBuffer("\"«÷€ÿ „‰ «·Ì”«— ··Ì„Ì‰\"", PRN_LEFT, SIZE_DEMI,0);
			PrintBuffer("«÷€ÿ ≈ ’«· #—ﬁ„ «·ÂÊÌ…*—ﬁ„ «· ›⁄Ì·*155*", PRN_LEFT, SIZE_DEMI,-5);

			PrintBuffer("·„⁄—›… «·—’Ìœ:", PRN_LEFT, SIZE_NORMAL_BOLD,0);
			PrintBuffer("\"«÷€ÿ „‰ «·Ì”«— ··Ì„Ì‰\"", PRN_LEFT, SIZE_DEMI,0);
			PrintBuffer("«÷€ÿ ≈ ’«· #166*", PRN_LEFT, SIZE_DEMI,0);
			PrintBuffer("·„“Ìœ „‰ «·„⁄·Ê„«  «·—Ã«¡ ≈ ’· ⁄·Ì 1500", PRN_LEFT, SIZE_DEMI,-5);
...
#ifdef FARSI
		sprintf(szBufferPrint, "%s%32s", "Expiration date", " «—ÌŒ «·’·«ÕÌ…");
#else
...
#ifdef FARSI
		sprintf(szBufferPrint, "%s%32s", "Serial Number", "«·—ﬁ„ «· ”·”·Ì");
#else 
...
// 			Prn_A_LineOf("*");
...
#ifdef FARSI
			sprintf(szBufferPrint, "%s%26s", "Lowest Prices For", "„ﬂ«·„«  œÊ·Ì…");
#else
...
#ifdef FARSI
			sprintf(szBufferPrint, "%s%26s", "International Calls", "»√—Œ’ «·√”⁄«—");
#else
		...
		if(PRN_CUSTOMER == Mode)
		{
...
#ifdef FARSI
			sprintf(szBufferPrint, "%s%26s", "Card Number", "—ﬁ„ «· ›⁄Ì·");
			PrintBuffer(szBufferPrint, PRN_MIDD, SIZE_DEMI,0);
			...
			sprintf(szBufferPrint, "%s%20s", "To make a Call dail", "·≈Ã—«¡ „ﬂ«·„… « ’·");
			PrintBuffer(szBufferPrint, PRN_MIDD, SIZE_DEMI,0);
			PrintBuffer_AR("8007008000", PRN_MIDD, SIZE_NORMAL_BOLD,0);
			//Jump_Lines(1);

			PrintBuffer("··œ⁄„ «·›‰Ì «·—Ã«¡ «·« ’«· »‹", PRN_LEFT, SIZE_DEMI,0);
...
static unsigned char Prn_SaleCodes_Quicknet(SAV_PARAM stSav, TRANS_DATA stData, unsigned char Mode)
{
	...
	do{
		...
		if(PRN_CUSTOMER == Mode)
		{
			...
			PrintBuffer_AR("ﬂÊÌﬂ ‰  «ﬂ”»—”", PRN_MIDD, SIZE_DEMI_BOLD,step);
		...
		if(PRN_CUSTOMER == Mode)
		{
			...
			PrintBuffer("⁄“Ì“Ì «·⁄„Ì· :", PRN_LEFT, SIZE_DEMI_BOLD,step);
			PrintBuffer("Ì„ﬂ‰ﬂ «⁄«œ… ‘Õ‰ —’Ìœ ‘—ÌÕ…", PRN_LEFT, SIZE_DEMI_BOLD,step);
			PrintBuffer("«·«‰ —‰  \"ﬂÊÌﬂ ‰ \" ⁄»— :", PRN_LEFT, SIZE_DEMI_BOLD,step);

			PrintBuffer("- («—”«· —”«·… ‰’Ì… «·Ï 1500  Õ ÊÌ ⁄·Ï", PRN_LEFT, SIZE_DEMI,step);
			PrintBuffer("#—ﬁ„ «·ÂÊÌ…*—ﬁ„ «⁄«œ… «·‘Õ‰*155*)", PRN_LEFT, SIZE_DEMI,step);
			PrintBuffer("(„‰ «·Ì”«— «·Ï «·Ì„Ì‰ )", PRN_LEFT, SIZE_DEMI,step);
			PrintBuffer("-  www.aljawal.net.sa “Ì«—… »Ê«»… «·ÃÊ«·", PRN_LEFT, SIZE_DEMI,step);
			PrintBuffer("- my.stc.com.sa “Ì«—… »Ê«»… Œœ„« Ì", PRN_LEFT, SIZE_DEMI,step);
			PrintBuffer("„Êœ„ ﬂÊÌﬂ ‰  ", PRN_LEFT, SIZE_DEMI,step);
...
static unsigned char Prn_SaleCodes_Virgin(SAV_PARAM stSav, TRANS_DATA stData, unsigned char Mode)
{
	...
	do{
		...
		if(PRN_CUSTOMER == Mode)
		{
			...
			Prn_A_LineOf("-");
#ifdef FARSI
			sprintf(szBufferPrint, "%s%31s", "To Top-up enter:", "·≈⁄«œ… «·‘Õ‰:"); // Modified by Kim_LinHB 2014/11/26
#else
			...
			sprintf(szBufferPrint, "%-46s#102*", "·„⁄—›… «·—’Ìœ:");
			SetPrintMode(0);
			...
			sprintf(szBufferPrint, "%-47s#112*", "·≈Œ Ì«— »«ﬁ… «·»Ì«‰« :");
			SetPrintMode(0);
			PrintBuffer(szBufferPrint, PRN_RIGHT, SIZE_DEMI,0);
			PrnStep(-30);
			PrnLeftIndent(50);
...
static unsigned char Prn_SaleCodes_FRiENDi(SAV_PARAM stSav, TRANS_DATA stData, unsigned char Mode)
{
	...
	do{
		...
		if(PRN_CUSTOMER == Mode)
		{
			...
			sprintf(szBufferPrint, "%s   *101#      %s", "To Recharge", "·≈⁄«œ… «·‘Õ‰");
			SetPrintMode(0);
			PrintBuffer(szBufferPrint, PRN_MIDD, SIZE_DEMI,0);
			PrnStep(-30);
			...
			sprintf(szBufferPrint, "%s  *102#      %s", "Check Balance", "·„⁄—›… «·—’Ìœ");
			SetPrintMode(0);
			PrintBuffer(szBufferPrint, PRN_MIDD, SIZE_DEMI,0);
			PrnStep(-30);
			PrnLeftIndent((384 - 20) / 2 + 25);
			...
			sprintf(szBufferPrint, "%s   *108#      %s", "Choose Data", "»«ﬁ«  «·»Ì«‰« ");
			SetPrintMode(0);
			PrintBuffer(szBufferPrint, PRN_MIDD, SIZE_DEMI,0);
			PrnStep(-30);
			PrnLeftIndent((384 - 20) / 2 + 25);
