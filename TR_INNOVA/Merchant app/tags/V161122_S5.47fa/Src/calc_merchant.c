...
static unsigned char AutoLinefeed(unsigned char *src);  //Added by Allen 2016-11-17  Auto Linefeed for print
...
//added by Allen 2016-11-17
static unsigned char AutoLinefeed(unsigned char *src)
{
	unsigned char *p,*pLatestBlank;
	int iLength = 1;
	int iWordLength = 1;
	if(src == NULL)
		return -1;
	...
	while(*p != 0)
	{
		if(*p == ' '&& iLength <= 30)    //29 characters each line
		{
			pLatestBlank = p;
			iWordLength = 0;
		}
		else if(*p == ' ' && iLength > 30 && iWordLength < 30)
		{
			*pLatestBlank = '\n';
			p = pLatestBlank;
			iLength = 0;
...
{
	unsigned char ucRet = OK;
	unsigned char ucArabicSlipMessage[200 + 1];  //added by Allen 2016-11-17 
	xml_t reqXML, headerXML, bodyXML;
	xml_t root = {"", pszXml, pszXml + uiLen, NULL};
		...
		while(ProductCategorys.start)
		{
			...
			if(ProductCategory.start != NULL)
			{
				...
				while(pstSav->ucProductsNum < NB_MAX_PRODUCTS)
				{
					...
					strcpy(pstSav->stSavParamProducts[pstSav->ucProductsNum].szSlipMessage, XmlGetValue(&Product, "SlipMessage"));
					AutoLinefeed(pstSav->stSavParamProducts[pstSav->ucProductsNum].szSlipMessage);
#ifdef FARSI
					memset(ucArabicSlipMessage, 0, sizeof(ucArabicSlipMessage));
					strcpy(ucArabicSlipMessage, XmlGetValue(&Product, "ArabicSlipMessage"));
					UTF8ToAr(ucArabicSlipMessage, strlen(ucArabicSlipMessage), pstSav->stSavParamProducts[pstSav->ucProductsNum].szSlipMessage_AR);
#endif
					//add end
