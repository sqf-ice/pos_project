	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				//added by Allen 2016-11-16 v5.45
				else if(strcmp(gl_MaintenaceParam_Ext.version, "5.45") < 0)
				{
					SAV_PARAM_OLD2 sav_old;
					PubFileRead(FILE_PARAM, 1 + sizeof(MAINTENACE_PARAM) + sizeof(TRANS_DATA), 
						(void *)&sav_old, sizeof(SAV_PARAM_OLD2));
					gl_SavParam.usTraceNo = sav_old.usTraceNo;
					gl_SavParam.usBatchNo = sav_old.usBatchNo;
					gl_SavParam.bIsSetup = sav_old.bIsSetup;
					gl_SavParam.bDownloadLogoToDo = sav_old.bDownloadLogoToDo;

					memcpy(gl_SavParam.ucKey, sav_old.ucKey, 16);
					memcpy(&gl_SavParam.stClaim, &sav_old.stClaim,sizeof(sav_old.stClaim));

					gl_SavParam.uiProductsListIndex = sav_old.uiProductsListIndex;

					gl_SavParam.ucCategoryNum = sav_old.ucCategoryNum;
					gl_SavParam.ucProductsNum = sav_old.ucProductsNum;

					for(i = 0; i < sav_old.ucCategoryNum; ++i)
					{
						gl_SavParam.stSavParamCategory[i].uiCodeCategory = sav_old.stSavParamCategory[i].uiCodeCategory;
						strcpy(gl_SavParam.stSavParamCategory[i].szName, sav_old.stSavParamCategory[i].szName);
						strcpy(gl_SavParam.stSavParamCategory[i].szName_AR, sav_old.stSavParamCategory[i].szName_AR);
						gl_SavParam.stSavParamCategory[i].ucCategoryType = sav_old.stSavParamCategory[i].ucCategoryType;
						gl_SavParam.stSavParamCategory[i].ucNumLogo = sav_old.stSavParamCategory[i].ucNumLogo;
					}

					for(i = 0; i < sav_old.ucProductsNum; ++i)
					{
						gl_SavParam.stSavParamProducts[i].uiCodeCategory = sav_old.stSavParamProducts[i].uiCodeCategory;
						gl_SavParam.stSavParamProducts[i].ucCategory_Index = sav_old.stSavParamProducts[i].ucCategory_Index;
						gl_SavParam.stSavParamProducts[i].ucType = sav_old.stSavParamProducts[i].ucType;
						strcpy(gl_SavParam.stSavParamProducts[i].szCodeProduct, sav_old.stSavParamProducts[i].szCodeProduct);
						strcpy(gl_SavParam.stSavParamProducts[i].szNameProduct, sav_old.stSavParamProducts[i].szNameProduct);
						strcpy(gl_SavParam.stSavParamProducts[i].szNameProduct_AR, sav_old.stSavParamProducts[i].szNameProduct_AR);
						strcpy(gl_SavParam.stSavParamProducts[i].szAmount, sav_old.stSavParamProducts[i].szAmount);
						strcpy(gl_SavParam.stSavParamProducts[i].szDiscount, sav_old.stSavParamProducts[i].szDiscount);
						gl_SavParam.stSavParamProducts[i].usNbMaxRestock = sav_old.stSavParamProducts[i].usNbMaxRestock;
						strcpy(gl_SavParam.stSavParamProducts[i].szSlipMessage, " ");
						strcpy(gl_SavParam.stSavParamProducts[i].szSlipMessage_AR, " ");
					}
					memset(gl_MaintenaceParam.szLastSettDownLoad_Time, 0, SIZE_DATE_TIME + 1);
					WriteParam(WRT_ALL);
				}
