...
static int ConnectPPP(unsigned char *pszPhone_nr, unsigned char *pszUser, \
					  unsigned char *pszPassword, unsigned char *pszIP, \
					  unsigned short usRemotPort, unsigned short usLocalPort);
...
static int P80TcpTxd(const unsigned char *psTxdData, unsigned short uiDataLen, unsigned short uiTimeOutSec);
static int P80TcpRxd(unsigned char *psRxdData, unsigned short uiExpLen, unsigned short *puiOutLen, unsigned short uiTimeOutSec);
#endif
...
void GetOperatorName()
{
 ...
 /*add by jeff_xiehuan_20170106 for getting IMSI_KEY and CCID_KEY*/
	...
	//DebugComSend("gl_SimMesg.IMSI=%s,gl_Simmesg.CCID_KEY=%s,gl_SimMesg.module_name=%s",gl_SimMesg.imsi,gl_SimMesg.ccid,gl_SimMesg.module_name);
...
int FTPLogin(FTP_PARAM stFTPParam)
{
	...
	if (iRet != 220)
	{
		...
		vTrace("Error:", &sReadBuff, strlen(sReadBuff), TRACE_ERROR, TYPE_S);
		return FTPSeverERR;
	}
...
int FTPGetFile(unsigned char *pszPCFile, unsigned char *pszPOSFile, unsigned char ucLineNo)
{
	...
	else
	{
		...
		sprintf(stConnParam_DATA.szRemotePort, "%u", usLocalProt);
	...
	while(lReadLen < lFileTotalLen)
	{
		...
		{
			lReadLen += uiRecvLen;
			iFileRet = PubFtpFileWrite(pszPOSFile, gl_stFTPFileInfo.lCurrLen,
				sReadBuff, (int)uiRecvLen);
			if (iFileRet != OK)
			{
				SetSocket(iSocket_DATA);
				TcpClose();
...
{
	unsigned char szFileName[256];
	unsigned char szPOSFileName[256];
	unsigned char szCmdBuf[100];
	unsigned char sReadBuff[1024*8];
	unsigned char *psFlag;
	unsigned char ucResumeFlag;
	...
	if (pszPCFile == NULL)
	{
		sprintf(gl_szFTPErrInfo, "%03d Put File Para ERR", FTPERROR);
		return FTPERROR;
	}
	...
	lFileTotalLen = filesize(szPOSFileName);
	if (lFileTotalLen < 0)
	{
		sprintf(gl_szFTPErrInfo, "%03d Get File Size Error[%ld]", FTPFileGetSizeERR, lFileTotalLen);
		return FTPFileGetSizeERR;
	}
	...
	memset(sReadBuff, 0, sizeof(sReadBuff));
	memset(szCmdBuf, 0, sizeof(szCmdBuf));
	sprintf(szCmdBuf, "%ld", lFileTotalLen);
	iRet = FTPCmd("ALLO ", szCmdBuf, sReadBuff);
	if (iRet != 200 && iRet != 202)
	{
		sprintf(gl_szFTPErrInfo, "%s", sReadBuff);
		//memcpy(gl_szFTPErrInfo, SeverError, 3);
		vTrace("Error:", &sReadBuff, strlen(sReadBuff), TRACE_ERROR, TYPE_S);
	...
	memset(sReadBuff, 0, sizeof(sReadBuff));
	iRet = FTPCmd("APPE ", psFlag, sReadBuff);//STOR ? APPE
	if (iRet != 150)
	{
		sprintf(gl_szFTPErrInfo, "%s", sReadBuff);
		//memcpy(gl_szFTPErrInfo, SeverError, 3);
	...
	lWriteLen = 0l;
	iCnt = 0;
	while(lWriteLen < lFileTotalLen)
	{
		iBlockLen = (1024<(lFileTotalLen-lWriteLen)) ? 1024 : (lFileTotalLen-lWriteLen);
		memset(sReadBuff, 0, sizeof(sReadBuff));
		iRet = PubFtpFileRead(szPOSFileName, gl_stFTPFileInfo.lCurrLen, sReadBuff, iBlockLen);
		if (iRet != OK)
		{
			SetSocket(iSocket_DATA);
			TcpClose();
		...
		iRet = Send(sReadBuff, (unsigned short)iBlockLen, gl_MaintenaceParam_Ext.cConnectionTimeout);
		if (iRet != OK)
		{
			SetSocket(iSocket_DATA);
			TcpClose();
		...
		lWriteLen += iBlockLen;
		gl_stFTPFileInfo.lCurrLen += iBlockLen;			
		WriteFTPFileInfo();
		if (ucLineNo < 6 && ucLineNo > 0)
		{
			if (ucResumeFlag ==1)
			...
			{
				DispRate(ucLineNo, gl_stFTPFileInfo.lTotalLen, gl_stFTPFileInfo.lCurrLen, gl_stFTPFileInfo.lCurrLen-iBlockLen);
			}	
		}		


		if (lWriteLen >= lFileTotalLen)
		{
			break;
		}
	}
...
int FTPCheckFile(unsigned char *pszPCFile)
{
	...
	{
		sprintf(gl_szFTPErrInfo, "%s", sReadBuff);
		//memcpy(gl_szFTPErrInfo, SeverError, 3); // Removed by Kim_LinHB 2013-7-10 v4.62
		vTrace("Error:", &sReadBuff, strlen(sReadBuff), TRACE_ERROR, TYPE_S); // Added by Kim_LinHB 2013-7-10 v4.62
		return FTPSeverERR;
	}
...
#else
	gl_MaintenaceParam.ucConnSel = 0;
	iRet = FTPConnectSub(uiTCPTimeOutSec, GPRS);
	if(0 == iRet)
	{
		return OK;
	}
...
unsigned char Send(const unsigned char *pData, unsigned int len, unsigned short uiTimeOutSec)
{
	...
	//DispSending(); // Removed by Kim_LinHB 2014/11/5
...
// 		}
	...
	{
#ifdef  LHB_DEBUG
		PubDebugOutput(NULL, "sent",strlen("sent"),DEVICE_COM1,ASC_MODE);
#endif
		return OK;
	}
	else
...
unsigned char Recv(unsigned int expLen, unsigned char *pDataOut, unsigned int *lenOut, unsigned short uiTimeOutSec)
{
	...
	//DispReceiving(); // Removed by Kim_LinHB 2014/11/5
...
static int ConnectSub(int iTimeout, unsigned char type)
{
	...
	case  GPRS:
		...
		gl_MaintenaceParam.stConnParam.ucIPUsing = 0;
#ifdef  LHB_DEBUG
		PubDebugOutput(NULL, "(tcp)connecting to host",strlen("(tcp)connecting to host"),DEVICE_COM1,ASC_MODE);
#endif
		...
		if(iRet != 0)
		{
			...
			{
#ifdef  LHB_DEBUG
				PubDebugOutput(NULL, "connected to host",strlen("connected to host"),DEVICE_COM1,ASC_MODE);
#endif
				vTrace("! SocketProcess KO", &iRet, sizeof(iRet), TRACE_ERROR, TYPE_N);
				iRet = GPRSConnectERR;
			}
		...
		break;
// 	case PPP:
// 		if(0 == gl_MaintenaceParam.stConnParam.ucTeleUsing)
// 		{
// 			iRet = ConnectPPP(gl_MaintenaceParam.stConnParam.szNumPad,gl_MaintenaceParam.stConnParam.stParamGPRS.szUID,
// 				gl_MaintenaceParam.stConnParam.stParamGPRS.szPwd,gl_MaintenaceParam.stConnParam.szRemoteIP,
// 				atoi(gl_MaintenaceParam.stConnParam.szRemotePort), 1250);
// 			if(iRet != OK)
// 			{
// 				if(strlen(gl_MaintenaceParam.stConnParam.szRemoteIP_Bak) > 0 &&
// 					strlen(gl_MaintenaceParam.stConnParam.szRemotePort_Bak) > 0)
...
// 					gl_MaintenaceParam.stConnParam.ucIPUsing = 1;
// 					iRet = ConnectPPP(gl_MaintenaceParam.stConnParam.szNumPad,gl_MaintenaceParam.stConnParam.stParamGPRS.szUID,
// 										gl_MaintenaceParam.stConnParam.stParamGPRS.szPwd,gl_MaintenaceParam.stConnParam.szRemoteIP_Bak,
// 										atoi(gl_MaintenaceParam.stConnParam.szRemotePort_Bak), 1250);
// 					if(OK == iRet)
// 					{
// 						break;
// 					}
...
// 			}
// 		}
// 		else if(1 == gl_MaintenaceParam.stConnParam.ucTeleUsing)
// 		{
// 			iRet = ConnectPPP(gl_MaintenaceParam.stConnParam.szNumPadSec,gl_MaintenaceParam.stConnParam.stParamGPRS.szUID,
// 				gl_MaintenaceParam.stConnParam.stParamGPRS.szPwd,gl_MaintenaceParam.stConnParam.szRemoteIP,
// 				atoi(gl_MaintenaceParam.stConnParam.szRemotePort), 1250);
// 			if(iRet != OK)
// 			{
...
static int TCPSocketProcess(CONN_PARAM *connParam, unsigned short uiTimeOutSec)
{
	...
	{
// 		EthSet(connParam->stParamEth.szLocalIP, connParam->stParamEth.szNetMask, 
// 			connParam->stParamEth.szGatewayIP, connParam->stParamEth.szDNS);
...
#ifdef _S_SERIES_
#ifdef  LHB_DEBUG
	PubDebugOutput(NULL, "GPRS init",strlen("GPRS init"),DEVICE_COM1,ASC_MODE);
#endif
	if(WlPppCheck() != 0)
	{
		iRet=WlSelSim(0);
...
// 		return OK;
	...
	for(i = 0; i < 3; ++i)
	{
		iRet = WNetMTcpConnect(&sg_iSocket, ip, port);
		iRet = WNetMTcpCheck(sg_iSocket);
		if(RET_TCPOPENED == iRet) 
		{
			return OK;
		}
					  ...
					  unsigned short usRemotPort, unsigned short usLocalPort)
{
	...
	while (iDialTimes < 2)
	{
		...
		if (0x00 == ucRet)  
			...
			ucRet = ModemDial(&tPPPModemPara,pszPhone_nr,1);
		if (ucRet != 0x00)
		{
			ModemReset();
			++iDialTimes;
...
#ifdef _S_SERIES_
	vTrace("PppLogin()", NULL, 0, TRACE_WARNING, 0);

	iRet = PPPLogin(pszUser, pszPassword, PPP_ALG_ALL, 0);
	if (iRet != 0)
	{
		vTrace("! PppLogin KO", &iRet, sizeof(iRet), TRACE_ERROR, TYPE_N);
		OnHook();		//v4.6
		return PPPLoginERR;
	}
	...
	vTrace("NetTcpConnect()", NULL, 0, TRACE_WARNING, 0);

	if(0 == gl_MaintenaceParam_Ext.cConnectionTimeout)
		gl_MaintenaceParam_Ext.cConnectionTimeout = TIME_OUT_CONN;

	iRet = SxxTcpConnect(pszIP, usRemotPort, gl_MaintenaceParam_Ext.cConnectionTimeout);
	if(iRet != OK)
	{
		vTrace("! NetTcpConnect KO", &iRet, sizeof(iRet), TRACE_ERROR, TYPE_N);
		OnHook();		//v4.6
		return PPPOpenERR;
	}
#else
	vTrace("NpppLogin()", NULL, 0, TRACE_WARNING, 0);

	iRet = NpppLogin(pszUser, pszPassword);
	if (iRet != 0)
	{
		vTrace("! NpppLogin KO", &iRet, sizeof(iRet), TRACE_ERROR, TYPE_N);
		OnHook();		//v4.6
		return PPPLoginERR;
	}
	...
	vTrace("NpppOpen()", NULL, 0, TRACE_WARNING, 0);

	iRet = NpppOpen(pszIP, usRemotPort, usLocalPort, 0);
	if (iRet < 0)
	{
		vTrace("! NpppOpen KO", &iRet, sizeof(iRet), TRACE_ERROR, TYPE_N);
		OnHook();		//v4.6
		return PPPOpenERR;
	}
...
#ifdef _S_SERIES_
#ifdef  LHB_DEBUG
	PubDebugOutput(NULL, "GPRS logging on",strlen("GPRS logging on"),DEVICE_COM1,ASC_MODE);
#endif
	do {
		iTimeoutNum++;
		for (i = 0;i < 3; ++i)
		{
			...
			{
#ifdef  LHB_DEBUG
				PubDebugOutput(NULL, "GPRS Logged on network",strlen("GPRS Logged on network"),DEVICE_COM1,ASC_MODE);
#endif
				iRet = OK;
				break;
			}
...
#else
	return P80TcpTxd(psTxdData, uiDataLen, uiTimeOutSec);
#endif
}
...
static int GPRSTxd(const unsigned char *psTxdData, unsigned short uiDataLen, unsigned short uiTimeOutSec)
{
	...
	return SxxTcpTxd(psTxdData, uiDataLen, uiTimeOutSec);
#else
	return WNetMTcpTxd(sg_iSocket, (unsigned char *)psTxdData, uiDataLen);
#endif
}
...
#else
	return P80TcpRxd(psRxdData, uiExpLen, (unsigned short *)puiOutLen, uiTimeOutSec);
#endif
}
...
static int GPRSRxd(unsigned char *psRxdData, unsigned short uiExpLen, unsigned int *puiOutLen, unsigned short uiTimeOutSec)
{
	...
	return SxxTcpRxd(psRxdData, uiExpLen, puiOutLen, uiTimeOutSec);
#else
	unsigned char Ret = WNetMTcpRxd(sg_iSocket, psRxdData, uiExpLen, (unsigned short *)puiOutLen, uiTimeOutSec * 1000);
	if(RET_NORSP == Ret)
	{
		*puiOutLen = 0;
		Ret = NoReponse;
...
static int GPRSClose(void)
{
	...
	TcpClose();
#ifdef  LHB_DEBUG
	PubDebugOutput(NULL, "closed tcp socket",strlen("closed tcp socket"),DEVICE_COM1,ASC_MODE);
#endif
	WlPppLogout();
#ifdef  LHB_DEBUG
	PubDebugOutput(NULL, "logged out GPRS network",strlen("logged out GPRS network"),DEVICE_COM1,ASC_MODE);
#endif
	return OK;
	//WlClosePort();
#else
...
// 			if (-13 == iCnt)
// 			{
// 				// Modified by Kim_LinHB 2013-8-19 v4.62
// 				if(iOffset > 0)
// 					iRet = 0;
// 				// Modify End
// 			}
...
// Added by Kim_LinHB 2013-4-10 v4.57
static int P80TcpRxd(unsigned char *psRxdData, unsigned short uiExpLen, unsigned short *puiOutLen, unsigned short uiTimeOutSec)
{
	unsigned int i = 0;
	unsigned char ucRet;
...
static void MatchOperatorName(const unsigned char *MCC_MNC, unsigned char *Name)
{
	...
	for(i = 0; i < size; ++i)
	{
		if(0 == memcmp(IMSILIST[i].Code, sTmp, 5))
		{
