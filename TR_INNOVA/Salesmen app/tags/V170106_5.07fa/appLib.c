			...
			if( pstMenu[ucCurItem].bVisible )
			{
				...
				else
				{
					...
					if (sg_stAppLibConfig.bStrFromRightToLeft)
					{
                        ...
                        sg_stAppLibConfig.pFnShowLeftRightString(ucLineNo, APPLIB_SHIFT_RIGHT|ucRever, ucBuff);
					}
					else
					{
						if( !bUseSno )
...
uchar PubDebugOutput(uchar *pszTitle, uchar *psData, ushort nDataLen, uchar ucDevice, uchar ucMode)
{
	...
	if (ucDevice&DEVICE_COM1 || ucDevice&DEVICE_COM2)
	{
		...
		ucPort = (ucPort==1 ? 3 : 0); // S80使用PINPAD口作为第二个COM口
#endif
		if (PortOpen(4, "9600,8,n,1") != 0)   // ucPort ->0
		{
			ucOverFlag = 1;
//			return 0xff;
		}
...
uchar PubDebugTx(uchar *psStr, ...)
{
	...
	memset(buffer, 0, sizeof(buffer));
	ucRet = PortOpen(serial_com, "115200,8,n,1");
	va_start(ap, psStr);
	vsnprintf(buffer, 2048, psStr, ap);
	va_end(ap);

	ucRet = PortSends(serial_com, buffer, strlen(buffer)+1);
	PortSend(0,'\r');
	PortSend(0,'\n');
	PortClose(serial_com);
	return ucRet;
}